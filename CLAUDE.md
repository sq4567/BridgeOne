...
### 2. 계획 변경 시 후속 Phase 문서에 직접 반영
- 진행 중 계획과 다르게 변경된 사항이 있으면, **현재 Phase가 아닌 영향받는 후속 Phase 문서에 직접 변경 내용을 작성**합니다
- 예: Phase 4.3.3 진행 중 생긴 변경이 Phase 4.4.2에 영향 → Phase 4.4.2 문서에 변경 사항 기록
- 이렇게 해야 후속 작업 시 변경된 맥락을 놓치지 않습니다
- **후속 Phase에 영향을 주는 수준의 변경**은 설계 문서(`docs/`)에도 함께 반영합니다 (내부 구현만 달라진 사소한 차이는 제외)
