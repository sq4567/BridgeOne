...
**구현 목표**: CP2102 장치 자동 감지 및 사용자 권한 요청 시스템
**예상 시간**: 40-50분
...
2. VID=0x10C4, PID=0xEA60 필터링으로 CP2102 장치 탐지
...
2. setup() 함수에서 UART 초기화:
   - 1,000,000 bps (1Mbps)
   - 8N1 설정 (SERIAL_8N1)
   - RX 핀: GPIO16, TX 핀: GPIO17 (Geekble nano ESP32-S3 기본 핀)
...
**검증 방법**:
- [ ] 두 태스크 정상 생성 확인 (로그)
- [ ] UART 수신이 별도 태스크에서 동작
- [ ] 디버그 정보 1초마다 출력
- [ ] CPU 코어 분산 확인 (Core 0, Core 1)
...
- [ ] USB 연결 성공 시 Connected 상태 표시
- [ ] 터치 이벤트 → 프레임 전송 → ESP32-S3 수신 확인
- [ ] 순번 카운터 순환 (0→255→0) 정상 동작
- [ ] 1분간 연속 전송 안정성 확인 (손실률 <1%)
...
1. platformio.ini에 Release 환경 추가:
   [env:release]
   extends = env:geekble_nano_esp32s3
   build_flags = 
       ${env:geekble_nano_esp32s3.build_flags}
       -O2                     ; 최적화 레벨 2
