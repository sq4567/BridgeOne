...
### ⚠️ Phase 3.3.3 구현에서 적용된 사항 (Phase 3.6 영향)

1. **ESP32-S3 지원 기능 목록이 확정됨 (`wheel`, `drag`, `right_click`)**:
   - Phase 3.3.3에서 `vendor_cdc_handler.c`의 `supported_features[]` 배열에 이 3가지 기능이 정의됨.
   - Phase 3.6.3에서 Android 측 모드별 동작 변경 시, 이 기능 목록을 기준으로 Essential/Standard 모드의 차이를 구현해야 함.
   - 현재 `multi_cursor`, `macro`, `extended_keyboard`는 ESP32-S3가 거부(reject)하므로, Android에서 이들 기능을 Standard 모드에서 활성화하더라도 실제로는 동작하지 않음 (Phase 4+에서 추가 예정).

### 📌 이전 Phase에서의 참고 사항

- **Phase 3.6은 `uart_handler.c` (ESP32)와 Android 쪽을 수정**: Phase 3.2.4에서 변경된 `vendor_cdc_handler.c`, `VendorCdcProtocol.cs`, `ConnectionViewModel.cs`와 겹치지 않으므로 영향 없음
- **역방향 알림 프레임은 고정 8바이트 바이너리**: JSON 파싱이나 null-terminate와 무관
