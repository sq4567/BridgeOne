---
title: "BridgeOne Phase 4.4: Page 2 — 풀 와이드 터치패드 (멀티 커서)"
description: "BridgeOne 프로젝트 Phase 4.4 - Standard 모드 Page 2: 풀 와이드 터치패드 + 멀티 커서(최대 4개) + 그리드 분할/직접 전환 버튼 레이아웃 + 소리 감지 전환"
...
# BridgeOne Phase 4.4: Page 2 — 풀 와이드 터치패드 (멀티 커서)
...
### 기존 구현
- `StandardModePage.kt`: 4페이지 HorizontalPager 구조 (Page 2는 `Page2AbsolutePointingPlaceholder()` → Phase 4.4에서 Page 2 = 멀티 커서로 재정의, 이전 Placeholder는 Page 3으로 이동)
...
## Phase 4.4.2: CursorModeButton 활성화 + CursorCountPopup + MultiCursorState ViewModel
...
5. **멀티 커서 비활성화** (멀티 → 싱글 복귀):
   ...
   - `deactivateMultiCursor()` 호출 → Windows 서버에 `hide_virtual_cursor` 전송 (Phase 4.4.6)
...
## Phase 4.4.4: 직접 전환 버튼 레이아웃 모드 (PadSwitchButtonPanel)
...
## Phase 4.4.5: 소리 감지 패드 전환 (AudioRecord)
...
## Phase 4.4.6: Windows 서버 연동 (show_virtual_cursor + 패드 경계 홀드 리셋)
