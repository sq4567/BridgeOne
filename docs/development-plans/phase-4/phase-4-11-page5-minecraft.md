---
title: "BridgeOne Phase 4.11: Page 5 — Minecraft 특화 페이지"
description: "BridgeOne 프로젝트 Phase 4.11 - Standard 모드 Page 5: Touchpad(시점) + DPad(이동) + 게임 액션 버튼"
...
# BridgeOne Phase 4.11: Page 5 — Minecraft 특화 페이지
...
## Phase 4.11.1: Page 5 레이아웃 및 DPad 기본 구현

**목표**: 좌측 터치패드+DPad, 우측 액션 패널의 2열 레이아웃 구현

> **⚠️ Phase 4.16(가로 지원) 대비**: 실제 가로 레이아웃은 Phase 4.16에서 일괄 추가한다. 좌(터치패드+DPad)/우(액션) 2열(`Row`) 비율을 가로에서 재배치하기 쉽도록 최상위 배치 방향을 한 곳에서 결정할 것. DPad는 정사각 유지가 필요하므로 방향 전환 시 크기 계산이 깨지지 않게 둘 것.
...
## Phase 4.11.2: DPad 고급 기능 (Sticky Hold, 드래그 전환, 대각선)
...
## Phase 4.11.3: Combat & Movement 버튼
...
## Phase 4.11.4: Inventory/Utility 및 Hotbar
