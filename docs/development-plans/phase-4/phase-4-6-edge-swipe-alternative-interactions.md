...
│   - 존 탭 → 액션 편집 패널로 전환
│   - 존 롱프레스 → 병합 / 분할 / 이동 / 삭제 메뉴
│     · 이동: ( 폭째 이동 / 액션만 교환 ) 토글 + ◀ 왼쪽 / 오른쪽 ▶ 로 한 칸씩 재정렬
├─ 액션 편집 패널 (선택된 존):
...
- [ ] 존 이동(재정렬): "폭째 이동" → 인접 존과 폭+액션 통째 교환, "액션만 교환" → 경계 고정 액션만 교환, 경계에서 해당 방향 버튼 비활성, Undo 단계별 복원 (NORMAL·SWIPE 양쪽)
