...
## Phase 4.8.4: 앱 재실행 시 종료 안내 토스트 잔상 제거

**개발 기간**: 0.5일 미만

**목표**: 뒤로가기를 두 번 눌러 앱을 종료한 뒤 다시 실행하면, 스플래시 또는 초기 화면 위에 `"BridgeOne을 종료하려면 한 번 더 터치하세요"` 토스트가 남아 표시되는 문제를 수정합니다.

**에뮬레이터 호환성**: 에뮬레이터에서 재현 및 검증 가능.

### 원인 추정

뒤로가기 두 번 처리 로직에서 첫 번째 뒤로가기 시 표시한 `Toast`가 Android 시스템 레벨에서 큐에 남아 있다가, 앱 재실행 후에도 뒤늦게 노출되는 것으로 추정됩니다.

### 구현 절차

1. **재현 확인** — 뒤로가기 두 번으로 종료 후 즉시 재실행하여 토스트 잔상 재현
2. **원인 코드 파악** — 뒤로가기 첫 번째 처리 코드(Toast 생성 위치) 확인
3. **수정** — `onPause` 또는 `onStop` 시점에 `toast.cancel()` 호출하거나, Android `Toast` 대신 앱 내 Compose 기반 토스트(`StatusToast` 등)로 교체하여 생명주기 제어
