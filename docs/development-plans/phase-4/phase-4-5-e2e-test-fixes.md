...
> **⚠️ Phase 4.5.13 변경사항**: 서버가 CDC 포트를 아예 찾지 못하는 근본 원인이 VID/PID 불일치임을 확인하고 수정 완료.
> `CdcConnectionService.cs`의 `TargetVid 0x303A → 0x04D9`, `TargetPid 0x4001 → 0x0024`으로 변경.
> 이로 인해 조사 B의 "COM 포트 자체를 못 여는" 케이스는 해결됨. 아래 조사는 포트를 열었음에도 통신이 성립하지 않는 잔여 케이스에 집중.

- 서버가 CDC 포트를 열었지만 통신이 성립하지 않는 경우 (CRC 오류, 프레임 유실 등)
