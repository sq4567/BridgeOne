...
**검증**:
- [ ] 일반 스크롤 모드에서 Y축(상하) 고정 후 손가락을 자유롭게 움직여도 스크롤이 Y축으로만 계속 작동 확인
- [ ] 무한 스크롤 모드에서 Y축 고정 후 동일하게 작동 확인
- [ ] X축(좌우) 고정 후 자유 이동 시 기존과 동일하게 잘 유지되는지 확인 (회귀 없음)
- [ ] Y축 스크롤 중 가이드라인이 Y축 방향으로만 계속 표시되는지 확인
- [ ] 스크롤 모드 외 기능(커서 이동, 클릭, 엣지 스와이프 등)에 영향 없음 확인
