...
## Phase 4.4.7: 엣지 스와이프 메뉴 등장/소멸 애니메이션
...
**목표**: Phase 4.4.5에서 즉시 표시/닫기로 구현된 엣지 스와이프 메뉴에 부드러운 등장/소멸 전환 효과를 추가합니다.
...
- [x] USB-OTG 연결 시 앱 자동 실행 및 USB 권한 요청 팝업 표시 확인
- [x] 권한 허용 후 ESP32-S3 동글이 PC에 HID 마우스로 인식되는지 확인
- [x] 앱 연결 상태 표시가 실제 연결 여부와 일치하는지 확인 (연결됨/끊김)
- [x] Android → ESP32-S3 → PC 경로로 커서 이동 패킷이 지연 없이 전달되는지 체감 확인

#### B. 커서 이동

- [x] 손가락 이동 방향과 PC 커서 이동 방향이 정확히 일치하는지 확인
- [x] 손가락을 천천히 이동할 때와 빠르게 이동할 때 커서 이동량 비율이 자연스러운지 확인 (포인터 다이나믹스 OFF 기준)
- [x] 터치패드 가장자리까지 이동한 뒤 손가락을 떼고 다시 올려도 커서가 자연스럽게 이어지는지 확인
- [x] 직각 이동(RIGHT_ANGLE) 모드에서 커서가 수평·수직축으로만 이동하는지 PC 화면에서 확인
- [x] 직각 이동 모드에서 대각선으로 긁을 때 실제로 커서가 축에 맞춰 고정되는지 확인

#### C. 클릭 동작

- [x] 짧은 탭 → 좌클릭 신호 전달 확인 (PC에서 클릭 반응)
- [x] 탭 인식 정확도: 일반적인 이동 중 의도치 않은 클릭이 발생하지 않는지 확인
- [x] 탭 인식 정확도: 가볍게 살짝 탭할 때 클릭 누락이 없는지 확인
- [ ] 우클릭 모드 전환 후 탭 → 우클릭 신호 전달 확인 (PC에서 우클릭 메뉴 등장)
- [x] 드래그(커서 이동) 중 손가락을 떼도 클릭 신호가 발생하지 않는지 확인

#### D. 스크롤 동작

- [x] 스크롤 모드 진입 후 손가락 상하 이동 → PC에서 실제 스크롤 확인
- [x] 스크롤 방향(상/하)과 PC 스크롤 방향이 일치하는지 확인
- [x] 무한 스크롤 모드에서 일정 속도 이상 이동 시 연속 스크롤 패킷이 전송되는지 PC 화면에서 확인
- [x] 무한 스크롤 속도에 따라 스크롤 속도가 실제로 빨라지는지 확인 (느린 이동 → 느린 스크롤, 빠른 이동 → 빠른 스크롤)
- [x] 무한 스크롤 속도에 따라 진동 강도가 달라지는지 실기기에서 체감 확인
- [x] 스크롤 모드에서 손가락을 뗐을 때 스크롤이 즉시 멈추는지 확인 (관성 없음)

#### E. DPI 제어

- [x] DPI 버튼으로 레벨 전환 시 커서 이동 속도 변화가 PC에서 실제로 체감되는지 확인
- [x] DPI HIGH: 동일 이동 거리에서 DPI LOW 대비 커서가 더 많이 이동하는지 확인
- [x] DPI 팝업에서 커스텀 배율 설정 후 커서 이동 속도가 즉시 반영되는지 확
- [x] 커스텀 DPI가 범위 경계(최솟값·최댓값)에 도달했을 때 경계를 넘어가지 않는지 확인

#### F. 포인터 다이나믹스 (커서 가속)

- [x] **Off 프리셋**: 손가락 속도에 관계없이 커서 이동량이 일정한지 (선형 이동) 확인
- [x] **Precision 프리셋 (WINDOWS_EPP)**: 느린 이동 시 가속 없이 정밀하게 이동하는지 확인
- [x] **Precision 프리셋 (WINDOWS_EPP)**: 빠른 이동 시 커서가 S커브 형태로 가속되어 이동 거리가 늘어나는지 확인
- [x] **Standard 프리셋 (WINDOWS_EPP)**: Precision보다 더 강한 가속감이 느껴지는지 확인
- [x] **Fast 프리셋 (LINEAR)**: 속도에 비례하여 커서 이동량이 선형으로 증가하는지 확인 (WINDOWS_EPP와 가속 곡선이 다르게 느껴지는지)
- [x] **maxMultiplier 상한**: Fast 프리셋에서 매우 빠른 스와이프를 해도 커서가 화면 전체를 한 번에 가로지르는 수준으로 튀지 않는지 확인
- [x] 스크롤 모드 활성 상태에서 다이나믹스 프리셋을 변경해도 스크롤 속도에 영향 없는지 확인
- [x] 직각 이동 모드에서 빠른 이동 시 가속이 적용되지 않는지 (커서가 축을 유지하며 선형 이동) 확인

#### G. DPI × 포인터 다이나믹스 중첩

- [x] DPI HIGH + Dynamics Standard 조합: DPI 단독 대비 빠른 이동에서 커서가 더 많이 이동하는지 확인 (DPI × 다이나믹스 배율 누적 적용)
- [x] DPI × 다이나믹스 배율이 중첩된 상태에서 매우 빠른 스와이프 시 HID 범위 상한(−127 ~ 127)으로 클리핑되어 커서가 비정상적으로 튀지 않는지 확인
- [x] DPI LOW + Dynamics Fast 조합: 배율이 서로 상쇄되어 자연스러운 속도 범위에서 동작하는지 확인
- [x] 적용 순서 확인: DPI 배율이 먼저, 다이나믹스 배율이 나중에 적용되는지 (같은 raw delta에서 DPI 고정 후 다이나믹스만 바꾸면 결과가 달라지는지 체감)
...
- [ ] `ControlButtonContainer` 버튼으로 변경한 모드 상태와 엣지 스와이프 메뉴의 강조 상태가 동기화되는지 확인
...
- [ ] 엣지 스와이프 메뉴 표시 중 `ModePresetButton` / `DynamicsPresetButton` 탭이 차단되는지 확인
