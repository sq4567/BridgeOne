---
title: "BridgeOne Phase 4.5: Page 3 — 키보드 중심 페이지"
description: "BridgeOne 프로젝트 Phase 4.5 - Standard 모드 Page 3: Modifiers, Navigation/Editing, Function Row, Shortcuts, Media, Lock Keys"
...
# BridgeOne Phase 4.5: Page 3 — 키보드 중심 페이지

**개발 기간**: 4.5-5.5일

**목표**: 키보드 입력과 단축키 트리거에 특화된 Page 3를 구현합니다. 터치패드 없이 키 입력에 집중하는 전용 인터페이스입니다. Phase 4.5.1에서 키 디스플레이 레지스트리를 먼저 구축하여, 이후 모든 키 배치에서 아이콘 자동 표시를 활용합니다.
...
- `KeyboardKeyButton.kt`: **Sticky Hold 이미 구현됨** (500ms 롱프레스 → 키 유지 → 재탭 해제)
  ...
  - **⚠️ Phase 4.2.3 완료**: Key Repeat 파라미터(`repeatEnabled`, `stickyHoldEnabled`, `repeatInitialDelayMs`, `repeatIntervalMs`, `onRepeatStart`) 이미 추가됨. Phase 4.5.3에서 `KeyboardKeyButton.kt` 추가 수정 불필요.
...
## Phase 4.5.1: 키 디스플레이 레지스트리 (KeyDisplayRegistry)
...
## Phase 4.5.2: Page 3 레이아웃 및 Modifiers Bar
...
> **⚠️ Phase 4.2.2 변경사항**: `StandardModePage.kt`에서 `KeyboardPage` Composable 및 `showKeyboard` 상태 관리 로직이 **완전 삭제됨**. 기존 코드에서 참조 불가. `Page3KeyboardCentric`은 `StandardModePage.kt`의 `HorizontalPager` page=2 분기에서 렌더링되며, 현재는 `Page3KeyboardPlaceholder()`로 대체 중. Phase 4.5 구현 시 이 placeholder를 `Page3KeyboardCentric` Composable로 교체하면 됨. 수정자 키 추적(`activeModifierKeys`)은 Page 2 내부에서 새로 선언해야 함.
...
## Phase 4.5.3: Navigation/Editing Grid

**목표**: 방향키 (Inverted-T) + 편집 키 그리드 구현

**개발 기간**: 1일

> **⚠️ Phase 4.5.1 변경사항**: `KeyDisplayRegistry`가 도입됨. 방향키, Backspace, Delete, Enter, Tab, Home, End 등 모든 편집/네비게이션 키는 `useRegistry = true`로 설정하여 자동 아이콘 표시를 활용할 것. `keyLabel` 수동 지정 불필요.
...
## Phase 4.5.4: Function Row 및 확장 Shortcuts
...
## Phase 4.5.5: Media Controls 및 Lock Keys

**목표**: 미디어 제어 버튼과 Lock Keys (HID LED 동기화)

**개발 기간**: 1일

> **⚠️ Phase 4.5.1 변경사항**: CapsLock 등 Lock Keys는 `KeyDisplayRegistry`에 아이콘이 등록되어 있으므로, `LockKeyButton` 구현 시 레지스트리를 활용하여 아이콘 표시 가능.
