...
| 순서 | 동작 | 예상 결과 | 확인 |
|------|------|---------|------|
| 1 | Windows 서버를 실행한다 | 서버 UI가 열린다 | [x] |
| 2 | 서버 UI를 관찰한다 | 핸드셰이크 완료 → **"Standard 모드" (녹색)** 표시 | [x] |
| 3 | 서버 UI에서 활성 기능 목록을 확인한다 | **"wheel, drag, right_click"** 표시 (녹색) | [x] |
| 4 | Android에서 **휠 스크롤**을 시도한다 | 메모장이 **스크롤된다** | [x] |
| 5 | Android에서 **우클릭 버튼**을 누른다 | PC에서 **우클릭 컨텍스트 메뉴가 나타난다** | [x] |
| 6 | Android에서 **일반 문자 키 (A, B 등)** 를 누른다 | 메모장에 **문자가 입력된다** | [x] |
| 7 | Android 터치패드에서 손가락을 움직인다 | PC 커서가 여전히 정상 움직인다 | [x] |
...
| 순서 | 동작 | 예상 결과 | 확인 |
|------|------|---------|------|
| 1 | Windows 서버 프로세스를 **강제 종료**한다 (작업 관리자 → BridgeOne.exe → 작업 끝내기) | 서버 UI가 닫힌다 | [x] |
| 2 | **즉시** Android 터치패드에서 손가락을 움직인다 | PC 커서가 **계속 움직인다** (HID는 서버 무관) | [x] |
| 3 | Android 앱 UI를 확인한다 | 우클릭, 휠 스크롤 등 고급 기능 UI가 **숨겨져** Essential 레이아웃으로 복귀 | [x] |
| 4 | Android에서 **방향키(↑↓←→)** 를 누른다 | 메모장에서 커서가 **이동한다** | [x] |
| 5 | Android 터치패드를 짧게 탭한다 (좌클릭) | 메모장에 커서가 찍힌다 | [x] |
...
| 순서 | 동작 | 예상 결과 | 확인 |
|------|------|---------|------|
| 1 | Android에서 **Shift 키를 누른 상태로 유지**한다 | 메모장에서 Shift 상태 표시 (대문자 입력 가능) | [x] |
| 2 | Shift를 **누른 채로** 서버를 강제 종료한다 | — | [x] |
| 3 | Android에서 Shift를 놓는다 | — | [x] |
| 4 | 서버를 다시 실행한다 (Standard 모드 전환) | — | [x] |
| 5 | Android에서 **일반 문자 키 (a)** 를 누른다 | 메모장에 **소문자 "a"** 가 입력된다 (Shift가 붙어있지 않음) | [x] |
...
**테스트 실행 날짜**: 2026년 3월 26일

| 테스트 | 검증 내용 | 결과 | 통과 | 비고 |
|--------|---------|------|------|------|
| 1. Essential 기본 동작 | 기본 기능 정상 동작, 고급 기능 UI 숨김 확인 | 정상 | [x] | |
| 2. Essential → Standard | 서버 연결 → 모든 기능 활성화 | 정상 | [x] | |
| 3. Standard → Essential | 서버 종료 → 기능 제한 즉시 재적용 | 정상 | [x] | |
| 4. 눌린 키 해제 | 전환 시 Shift 등 수정자 키 자동 해제 | 정상 | [x] | 모드 전환 중 터치패드 커서 일시 정지는 정상 동작 (Compose recompose로 인한 터치 세션 리셋) |

**전체 통과 여부**: [x] PASS / [ ] FAIL

**발견 사항 및 개선 필요사항**:
- 모드 전환 시 터치패드 커서 이동이 일시 멈춤 → Compose recompose로 인한 정상 동작, 손가락을 떼었다 다시 터치하면 즉시 재개
