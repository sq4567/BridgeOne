...
PING 전송 → 1초 내 PONG 미수신 → 실패 카운트 +1
  ├─ 실패 1회: 경고 로그
  ├─ 실패 2회: UI에 "연결 불안정" 표시
  └─ 실패 3회: 연결 끊김 판정 → "연결 없음" 전환 + 자동 재연결 루프 시작
...
**확인 항목**:
- [x] Windows UI에서 ESP32 모드 "Standard 모드" (녹색) 표시
- [x] Windows UI에서 RTT 값 표시 (예: `RTT: 5ms (평균 4.2ms)`)
- [x] Windows UI에서 연결 품질 "양호" 표시
- [x] PING/PONG 반복 계속 유지 (중단 없음)
- [x] 로그 폭주 없음 (DEBUG 로그가 과도하지 않음)
...
**목표**: ESP32↔PC USB 케이블 분리 시 Windows 서버가 PONG 타임아웃을 감지하여 1.5~3초 내 "연결 없음" 전환 → 자동 재연결 루프 시작

> ℹ️ **모드 개념 정리**:
> - **Essential 모드**: ESP32↔PC 하드웨어 연결 중이지만 서버 미실행 (HID만) → **Android 앱 + ESP32 보드 전용 개념**
> - **Windows 서버 기준 모드**: `Standard 모드` / `연결 없음(Disconnected)` 두 가지만 존재
> - 케이블 분리로 인한 연결 끊김은 Windows 서버 입장에서 "연결 없음"으로만 표시됨

**준비 단계**:
1. 정상 동작 상태 확인 (Windows UI: Standard 모드 녹색)
2. **스톱워치 준비**
3. ESP32↔PC 연결 케이블 위치 확인 (분리할 케이블)

**테스트 진행**:
1. **ESP32↔PC USB 케이블 분리** (Android↔ESP32 케이블은 그대로 유지)
2. **즉시 스톱워치 시작** → Windows UI 관찰
3. **예상 순서 (Windows 로그)**:
   - T = 0~1초: PONG 미수신 1회 → 로그: `[Keep-alive] PONG 미수신 (1회)`
   - T = 0.5~2초: PONG 미수신 2회 → 품질 "불안정", 로그: `[Keep-alive] PONG 미수신 (2회) - 연결 불안정`
   - T = 1.5~3초: PONG 미수신 3회 → 연결 끊김 판정, 로그: `[Keep-alive] 연속 3회 PONG 미수신 → 연결 끊김 판정`
4. **전환 후 확인**:
   - Windows UI: "연결 없음" (회색) 전환
   - Windows UI: 재연결 메시지 표시 (형식: `재연결 중... (시도 #1, 1초 대기)`)

**실제 측정**:
- [x] 케이블 분리 후 Windows UI에서 "연결 없음" 전환까지 소요 시간: 1.6초
- [x] Windows UI에서 재연결 메시지 표시 확인 (형식: `재연결 중... (시도 #1, 1초 대기)`)
- [x] Windows 로그에서 품질 "불안정" → "끊김" 단계적 변화 확인

**성공 기준**:
- "연결 없음" 전환까지 **1.5~3초** (PING 500ms 주기 + PONG 1초 타임아웃 × 3회)
- Windows UI에서 재연결 시도 메시지 표시
- 품질 표시가 양호 → 불안정 → 끊김 순으로 단계적으로 변화

**롤백 기준**:
- 3초 이상 소요 또는 타임아웃 미감지 → `KeepAliveService.cs`의 `OnPongFailed()` 로직 재검토
...
> ℹ️ **로그 확인 방법**: DTR=false 순간에는 CDC 연결이 끊기므로 전환 로그를 실시간으로 볼 수 없다.
> 대신 펌웨어가 Tera Term **연결 시점에 현재 상태를 자동 출력**하므로, 연결 후 즉시 `Current state: IDLE`을 확인한다.
> 추가로 `status` 명령어를 입력하면 언제든 현재 상태를 재확인할 수 있다.
>
> **타이밍 측정 불필요**: DTR=false 콜백에서 즉각적으로 `connection_state_reset()`을 호출하므로
> 전환 시간은 항상 < 1ms이다. (< 500ms 기준은 코드 구조상 자동 충족)

**준비 단계**:
1. 정상 동작 상태 확인 (서버 연결됨, PING 정상)
2. Tera Term 실행 — 아직 연결하지 않고 대기
3. Windows 작업 관리자 준비 (또는 PowerShell에서 `taskkill /IM BridgeOne.exe /F`)

**테스트 진행**:
1. **Windows 서버 프로세스 강제 종료**
   - 작업 관리자 → BridgeOne.exe → 작업 끝내기
   - 또는 PowerShell: `taskkill /IM BridgeOne.exe /F`
2. **Tera Term으로 COM 포트(COM8 등) 연결** (115200 baud)
   - 서버가 종료되면서 COM 포트가 해제되므로 연결 가능
3. **연결 직후 자동 출력 확인**:
   - 연결 배너 다음 줄에 `Current state: IDLE` 출력 여부 확인
   - 출력 예시:
     ```
     ...
     Current state: IDLE
     ```
4. **(선택)** Tera Term에서 `status` 입력 → `Current state: IDLE` 재확인

**실제 측정**:
- [x] Tera Term 연결 후 `Current state: IDLE` 자동 출력 확인
- [x] `status` 명령어 입력 시 `Current state: IDLE` 응답 확인

**성공 기준**:
- Tera Term 연결 시 `Current state: IDLE` 자동 출력
- USB 포트 유지 (Tera Term 연결 자체가 성공하면 확인됨)
- 서버 재시작 시 정상 재연결 가능

**롤백 기준**:
- Tera Term 연결 시 `Current state: CONNECTED` 또는 기타 상태 출력 → ESP32 DTR 감지 코드 재검토

---

### 🔁 테스트 4: 케이블 재연결

**목표**: 테스트 2에서 분리된 ESP32↔PC 케이블 재연결 → Windows 서버 재연결 루프 감지 → 핸드셰이크 → Standard 모드 복귀

**준비 단계**:
1. 테스트 2 완료 상태 (ESP32↔PC 케이블 분리, Windows 서버 재연결 루프 진행 중)에서 시작
2. 분리된 ESP32↔PC USB 케이블 준비
3. Windows 로그 모니터링 상태 확인

**테스트 진행**:
1. **ESP32↔PC USB 케이블 재연결** (완전히 삽입될 때까지 기다림)
2. **스톱워치 시작**
3. **Windows 로그 관찰**:
   - 다음 재연결 시도 사이클에서 CDC 포트 감지
   - 핸드셰이크 완료 로그 (예: `[핸드셰이크] 성공`)
   - Keep-alive 재시작 로그 (예: `[Keep-alive] 자동 시작됨`)
4. **타이밍 측정**:
   - 케이블 삽입부터 Standard 모드 복귀까지 소요 시간

**실제 측정**:
- [x] 케이블 재연결 후 Windows UI "Standard 모드" (녹색) 복귀 확인
- [x] Standard 모드 복귀까지 소요 시간: 1.5초
- [x] RTT 값 재표시 확인

**성공 기준**:
- Windows 서버 재연결 루프가 자동으로 CDC 포트 감지 ✓
- 핸드셰이크 자동 수행 ✓
- Windows UI에서 "Standard 모드" (녹색) 복귀 ✓
- Windows UI에서 RTT 재표시 ✓

**롤백 기준**:
- 재연결 루프가 포트를 감지하지 못함 → `KeepAliveService.ReconnectLoopAsync()`의 포트 스캔 로직 확인
- 핸드셰이크 반복 실패 → `HandshakeService.PerformHandshakeAsync()` 안정성 확인
...
|------|------------|----------|--------|-------------|------|
| 1 | 1초 | ___초 | ___초 | `재연결 중... (시도 #1, 1초 대기)` [x] | [x] Success / [ ] Fail |
| 2 | 2초 | - | - | - | 시도 1에서 성공 |
| 3 | 4초 | - | - | - | 시도 1에서 성공 |
...
**테스트 실행 날짜**: 2026년 03월 23일

| 테스트 항목 | 예상 결과 (Windows UI) | 실제 결과 | 통과 | 비고 |
|-----------|--------|--------|------|------|
| 1. 정상 동작 | Standard 모드 🟢, RTT < 10ms | 정상 | ✓ | |
| 2. 케이블 분리 | 연결 없음 ⚫ (1.5~3초 내) + 재연결 루프 | 1.6초 | ✓ | |
| 3. 서버 종료 | ESP32 IDLE 전환 (ESP32 시리얼 모니터 확인) | 정상 | ✓ | |
| 4. 케이블 재연결 | Standard 모드 🟢 복귀 (< 10초) | 1.5초 | ✓ | |
| 5. 서버 재시작 | Standard 모드 🟢 복귀 (지수 백오프) | 정상 | ✓ | 핸드셰이크 로그 다량 출력됨 (동작 자체는 정상) |

**전체 통과 여부**: [x] PASS / [ ] FAIL

**발견 사항 및 개선 필요사항**:
- 테스트 5에서 서버 재시작 시 핸드셰이크 메시지가 다량 출력되어 로그 가독성 낮음 (기능 이상 없음)
