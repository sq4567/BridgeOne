...
### 3.10 Orbit Mini 연동 및 매크로 실행 시스템

> **개요**: Windows 서버는 ESP32-S3로부터 수신한 매크로 실행 요청을 외부 매크로 실행 엔진인 Orbit Mini로 중계하고, 실행 결과를 다시 ESP32-S3로 전달하는 중계 역할을 수행합니다.
...
**Orbit Mini → Windows (매크로 목록 응답)**:

```json
{
  "message_id": "uuid-v4",
  "command": "MACRO_LIST_RESULT",
...
**필드 설명**:
- `payload.macros`: 현재 Orbit Mini에 등록된 매크로 목록 (배열)
  - `macro_id`: UUID v4 형식의 매크로 고유 식별자
  - `name`: 사용자에게 표시할 매크로 이름
  - `description`: 매크로 설명 (선택적, 빈 문자열 가능)
  - `is_active`: 매크로 활성화 여부. `false`인 경우 Android 앱 선택 팝업에서 제외

> **Note**: `MACRO_LIST` 명령은 Android 앱의 MacroButton 롱프레스에 의해 트리거됩니다. Windows 서버는 Android로부터 `MACRO_LIST_REQUEST`를 수신하면, Orbit Mini에 `MACRO_LIST` 명령을 전송하고 결과를 `MACRO_LIST_RESPONSE`로 다시 Android에 전달합니다. 목록 조회 타임아웃: 5초.
...
**매크로 목록 조회 경로 (MACRO_LIST_REQUEST)**:

1. **Vendor CDC 수신**: ESP32-S3로부터 `MACRO_LIST_REQUEST` JSON 수신
2. **Orbit Mini 연결 상태 확인**:
   - `isOrbitConnected == false`: 즉시 `ORBIT_NOT_CONNECTED` 오류 응답
   - `isOrbitConnected == true`: 다음 단계 진행
3. **Named Pipe 전송**: Orbit Mini에 `MACRO_LIST` 명령 전송 (§3.10.2 구조 준수)
4. **응답 수신 및 중계**: Orbit Mini로부터 `MACRO_LIST_RESULT` 수신 → `MACRO_LIST_RESPONSE`로 변환 → Vendor CDC → ESP32-S3 → Android
   - 5초 타임아웃: 응답 없으면 `MACRO_LIST_TIMEOUT` 오류 응답

---

**매크로 실행 요청 경로 (Vendor CDC → Named Pipe)**:
...
- **자동 재연결**:
  ...
  - 최대 3회 재시도, 실패 시 수동 Orbit Mini 재시작 필요
...
// Orbit Mini 연결 시 isOrbitConnected = true 설정
...
#### 3.10.6 Orbit Mini 클라이언트 요구사항

> **Note**: Orbit Mini은 BridgeOne과 독립적인 외부 프로그램입니다. 상세 구현은 Orbit Mini 전용 문서를 참조하세요.
...
|------------|---------|---------|---------|----------------|
| Orbit Mini 미연결 | Windows 서버 | `ORBIT_NOT_CONNECTED` | 즉시 | 즉시 ESP32-S3로 오류 응답 전송 |
...
- **오류 응답**: Orbit Mini 미연결 감지 → Vendor CDC 오류 응답 전송까지 10ms 이내
...
> **개요**: BridgeOne은 두 가지 매크로 실행 방식을 지원합니다. 본 §3.10이 다루는 **Software Macro (Orbit Mini 연동)**와, Windows 서버를 경유하지 않는 **Native Macro (HID 직접 전송)**입니다. 두 방식은 독립적으로 동작하며, 사용자가 매크로 생성 시 방식을 선택합니다.
...
| **매크로 저장 위치** | Orbit Mini | ESP32-S3 NVS Flash (최대 64개) |
