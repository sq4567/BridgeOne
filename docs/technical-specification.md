...
graph TD
    subgraph "물리적 연결 구조"
        ...
        OTG_Y -- "Device 포트" --> ESP32_UART["ESP32-S3 DevKitC-1<br/>UART 포트 (내장 USB-Serial)"]
        OTG_Y -- "Power 포트" --> Charger["충전기<br/>(5V 2A+)"]
        ESP32_UART -- "GPIO43/44<br/>(UART, 1Mbps, 8N1)" --> ESP32_S3["ESP32-S3-WROOM-1-N16R8<br/>(16MB Flash, 8MB PSRAM)"]
        ESP32_S3 -- "USB 포트<br/>(Native USB OTG, 복합 장치)" --> PC["PC<br/>(Windows 10/11)"]
    end
...
- **ESP32-S3 DevKitC-1**: UART 포트 (보드 내장 USB-Serial 칩)
- **ESP32-S3**: ESP32-S3-WROOM-1-N16R8 모듈, USB 복합 장치 구성
...
#### 2.3.1 ESP32-S3 DevKitC-1 보드 내장 USB-Serial

**개요:**
- **역할**: USB-UART 브릿지로 Android와 ESP32-S3 간 직렬 통신 중계
- **구성**: ESP32-S3 DevKitC-1 보드에 내장된 USB-Serial 칩
- **주요 특징**:
  - 보드의 UART 포트를 통해 GPIO43/44와 연결
  - 외부 어댑터 불필요, 직접 보드 포트 사용
  - 3.3V TTL 레벨 신호 (ESP32-S3 네이티브 호환)
  - 안정적인 연결과 간편한 사용성

**성능 최적화 포인트:**
- **통신 속도**: 1Mbps UART 설정 (보드 내장 칩 지원)
- **신호 무결성**: 보드 내장 경로로 노이즈 최소화
- **전력 관리**: 보드 통합 설계로 효율적인 전력 관리

#### 2.3.2 ESP32-S3-WROOM-1-N16R8
...
- **주요 특징**:
  ...
  - 16MB Flash, 8MB PSRAM (PSRAM은 추후 대용량 버퍼링 및 고급 기능 확장 시 활용 예정)

**성능 최적화 포인트:**
- **USB 스택**: Arduino USB 라이브러리 기반으로 경량화된 USB 복합 장치 구현
- **실시간 처리**: FreeRTOS 기반으로 HID 프레임 처리 최적화
- **메모리 관리**: 512KB SRAM (16MB Flash, 8MB PSRAM 확장 가능)으로 효율적인 버퍼 관리
- **전력 최적화**: 저전력 모드 지원으로 배터리 사용 시간 연장

#### 2.3.3 하드웨어 간 통신 최적화 전략

**UART 통신 (Android ↔ ESP32-S3, 보드 내장 USB-Serial 경로):**
...
**전력 관리:**
- **보드 내장 USB-Serial**: 저전력 모드 (절전 시 최소 전력)
...
| 메모리 사용률 | ≤ 70 | % | 512KB SRAM 기준 (PSRAM 추후 활용) | 힙 모니터링 |
...
**핵심 구현 영역**:
- **USB 통신 아키텍처**: 보드 내장 USB-Serial 기반 1Mbps UART 통신, BridgeOne 프로토콜 구현, HID/Vendor CDC 인터페이스 최적화
...
| 용어 | 설명 | 사양 | 역할 |
|------|------|------|------|
| **ESP32-S3 DevKitC-1 보드 내장 USB-Serial** | 보드 내장 USB-UART 브리지 | 3.3V TTL, GPIO43/44 연결 | Android ↔ ESP32-S3 직렬 통신 |
| **ESP32-S3-WROOM-1-N16R8** | 메인 마이크로컨트롤러 | 16MB Flash, 8MB PSRAM, 듀얼 코어 240MHz | USB 복합 장치 및 HID 처리 |
