...
**오류 처리:**
- **보안 시퀀스**: `CTRL+ALT+DEL`, `WIN+L` 등 시스템 시퀀스는 실행 실패 처리
...
**매크로 실행 플로우:**
- **실행 요청**: `MACRO_START_REQUEST(macroId)`를 USB-Serial을 통해 PC Windows 서비스로 전송
- **즉시 비활성화**: 요청 전송 후 `isEnabled = false`로 전환하여 중복 실행 방지
- **응답 대기**: `onNewData()` 콜백으로 `TASK_COMPLETED` 또는 `MACRO_NOT_FOUND` 수신 대기
...
- **취소 요청**: `MACRO_CANCEL_REQUEST(macroId)` 전송으로 서버 측 매크로 중단
- **강제 활성화**: `UI_FORCE_ENABLE_ALL_TOUCHABLES_REQUEST` 전송으로 전체 UI 복원

**상태별 사용자 피드백:**
- **성공**: "#4CAF50" 녹색 토스트 + 성공 햅틱 피드백 + 자동 활성화
- **실패**: "#F44336" 빨간색 토스트 + 오류 햅틱 피드백 + 자동 활성화
- **취소**: "#FF9800" 주황색 토스트 + 중간 햅틱 피드백 + 즉시 활성화
- **진행 중**: "#2196F3" 파란색 토스트 (1000ms 간격) + 로딩 애니메이션
