...
**레이아웃 모드** (`CursorModeButton` 롱프레스로 토글, Phase 4.8.4):
...
**패드별 프리셋 자동 시드 (Phase 4.8.8)**:
- 멀티 커서 활성화 시 각 패드는 `CursorCountSelectionPopup`의 패드별 프리셋 지정 단계(`docs/android/component-touchpad.md` §1.7.2)에서 확정한 매핑(패드 인덱스 → `MODE_PRESETS` 인덱스, 신규 프리셋 정의 없이 §2.2.7 목록 재사용)으로 시드된다.
- 매핑을 한 번도 확정하지 않았으면(세션 초기값) 모든 패드가 **싱글 커서 모드에서 현재 사용 중인 프리셋**(`TouchpadState.modePresetIndex`)으로 시작한다. 확정 이력이 있으면 그 값이 세션 동안 유지되며(영속 없음), 앱 재시작 시 다시 싱글 커서 프리셋 추종으로 복귀한다.
- 패드가 미지정 상태이면: 이미 활성 중이던 패드는 그 `PadModeState`를 보존하고, 신규 패드이거나 애초에 비활성이었다면 싱글 커서의 현재 `PadModeState`를 시드로 사용한다(하위 호환).
