...
- 조합 표기: 각 단축키 기능을 나타내는 Material Icon(20dp)으로 시각화 (예: 복사=ContentCopy, 붙여넣기=ContentPaste). 누름 순서 충돌 방지 로직은 `docs/component-design-guide.md` §2.2 참조.
...
- Compose: Pager/Indicator는 본 페이지에서 `Selected` 고정, 네비게이션 제스처는 `Enabled`(페이지 전환 제스처는 §8.2.1 페이지 전환 방식 정책 준수).
