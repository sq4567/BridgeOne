...
graph TD
    subgraph "WaitingForConnection 플로우"
        ...
        Connection_Waiting -- "1단계" --> USB_Detection["USB 장치 자동 탐지<br/>(USB-Serial 장치)"]
...
1. **장치 탐지 단계**
  - "USB 동글 확인 중..." 메시지 표시
  - USB-Serial 장치 자동 인식 및 드라이버 호환성 확인
2. **권한 처리 단계**
...
**구현 핵심 요소:**
- **자동 탐지**: USB-Serial 장치 BroadcastReceiver 기반 실시간 감지
