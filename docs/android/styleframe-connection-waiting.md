...
- 배경: 방사형 그라데이션 (중앙 #1E1E2E → 외곽 #121212, 반경: 화면 최소 변의 70%)
- 페이지 인디케이터: 숨김 처리
- 네비게이션: 비활성화 (스와이프 제스처 무시)
- 상단: BridgeOne 별 로고 (32dp, #2AA9FF, alpha 0.6)
- 하단: "BridgeOne" 브랜딩 텍스트 (14sp, #404040)
...
- **연결 아이콘:**
  ...
  - 펄스 링: 아이콘 주변 동심원 2개, 3초 주기 확산 (시차 1.5초), #FF9800 alpha 0.3→0.0, stroke 2dp
- **주 메시지 (동적 변경):**
  - **1단계**: "USB 동글을 연결해주세요"
...
- **연결 단계 스텝 인디케이터:**
  - 위치: 부 메시지 하단 32dp
  - 구성: 점 3개 + 연결선 (●───○───○)
  - 점 크기: 8dp, 간격: 24dp (선 포함)
  - 활성 색상: #FF9800, 비활성 색상: #404040
  - 연결선: 1dp, #404040
  - 전환: 점 색상 CrossFade (300ms)
  - 단계 매핑: WaitingForUsb=1, SearchingServer=2, PermissionRequired/Error=3
...
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
  배경: 방사형 그라데이션 (#1E1E2E 중앙 → #121212 외곽)
```
...
**USB 아이콘 주변 동심원 확산 효과:**
- **링 개수**: 2개 (시차 1.5초)
- **확산 범위**: 아이콘 반경의 50% → 150% (64dp → 192dp)
- **주기**: 3초 (infiniteRepeatable)
- **색상**: #FF9800 (StateWarning), alpha 0.3 → 0.0 (확산하며 소멸)
- **두께**: 2dp stroke
- **조건**: `isProcessing = true`일 때만 표시 (권한 요청/오류 시 비표시)
- **접근성**: 장식적 요소, contentDescription 없음
- **애니메이션 비활성**: `TRANSITION_ANIMATION_SCALE = 0` 시 숨김

### 4.3 상태 전환 애니메이션 (스타일프레임 규격)
