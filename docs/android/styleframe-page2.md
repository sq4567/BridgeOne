...
멀티 커서 활성화 + `DIRECT_BUTTON` 레이아웃 모드일 때:
...
│   │   ┌──────────────────────────────────────────────────────────────┐   │   │
│   │   │  [★ Pad 1]  │    [Pad 2]    │    [Pad 3]    │  ...          │   │   │
│   │   └─────── PadSwitchButtonPanel (하단 48dp 오버레이) ─────────────┘   │   │
│   └──────────────────────────────────────────────────────────────────────┘   │
└──────────────────────────────────────────────────────────────────────────────┘
```

`PadSwitchButtonPanel` 시각 토큰 (Phase 4.8.4, component-touchpad.md §1.2.2 기준으로 통일):
- 높이: 48dp, 전체 너비, N등분
- 활성 버튼: 파란색 배경 (`#2196F3`) + 볼드 텍스트
- 비활성 버튼: 동일 색 반투명 배경 (`alpha = 0.4f`)
- 모서리: 패널 외곽만 `RoundedCornerShape(8dp)`, 버튼 사이는 붙어 있고 각짐 (터치패드 본체의 둥근 모서리와 시각적 통일)
