...
> **⚠️ 설계 변경(2026-07-09, 유저 확정)**: 패드 비율은 Page 3 자체가 아니라 Phase 4.15 페이지 커스터마이징의 그리드 배치가 소유한다. 실시간 프리셋 토글은 두지 않는다("이 지점을 누르면 저기쯤 커서가 간다"는 위치 감각 유지를 위해 한 번 정한 비율을 빠르게 바꾸지 않음). 자세한 근거는 `technical-specification-app.md` §2.10.7 참조.

- **비율**: 4.15 완성 전까지 **Fill**(가용 공간 전체) 고정. 4.15 완성 후에는 그리드 편집 화면에서 셀 크기(자유 비율 또는 모니터 종횡비 일치)를 정하고 유지
...
- **정의 중 직사각형 프리뷰**: 정의/확정 대기 중 `rectFromCenterDrag` 기반 PC 매핑 직사각형을 실시간 프리뷰(멀티 존과 동일 스타일, Phase 4.9.12). 배율 스칼라 텍스트("2.0x")는 임의 종횡비 직사각형 정의로 전환되며 표시할 값이 없어 폐지
- **정의 드래그 중심점 표시**: 흰 점 + 오렌지 테두리로 DOWN 시작 위치를 표시(멀티 존 `zoneCenterPoint`와 동일 패턴)
- **테두리 색상**: 줌 활성 시 전용 색상 `#FF9800` (주황색)으로 전환
- **줌 진입 중 (드래그 단계)**: 드래그에 따라 직사각형이 실시간으로 변하는 피드백(중심점 고정, finger가 멀어질수록 영역이 커짐 = 저배율)
