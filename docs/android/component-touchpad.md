...
- **선택 동작**: 버튼 탭 → 즉시 팝업 닫힘 + 선택한 커서 수로 멀티 커서 활성화
- **취소 동작**: 팝업 외부 터치 → 팝업 닫힘, 싱글 커서 유지 (모드 변경 없음)
...
- 우측에 빨간색 "해제" 버튼이 추가로 표시된다.
- 강조되지 않은 수를 선택하면 해제 없이 커서 수만 변경된다(`MultiCursorController.changeCursorCount`) — 기존 패드별 모드 상태(`padModeStates`)는 보존되며, 늘어난 패드는 pad1 상태로 시드되고 줄어든 패드는 뒤에서부터 절단된다.
- "해제" 탭 시에만 싱글 커서로 복귀한다.
