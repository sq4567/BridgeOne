...
#### 1.2.1 터치패드 영역 (TouchpadArea, pad1~padN)

`TouchpadAreaWrapper` 내부의 터치 인터페이스 영역:
- **싱글 커서 모드**: `TouchpadAreaWrapper` 전체 크기를 pad1 하나가 사용
- **멀티 커서 모드 (그리드 분할)**: 커서 수(N, 최대 4개)에 따라 자동 분할
  - N=2: 좌/우 50% (1×2)
  - N=3: 좌/중/우 33% (1×3)
  - N=4: 2×2 그리드 (각 셀 50%×50%)
- **멀티 커서 모드 (직접 전환 버튼)**: 터치패드 전체 면적 유지, 하단에 N개 전환 버튼 표시
- 기능: 터치 포인트 감지 및 처리, 커서 이동, 스크롤 동작
- 테두리: 멀티 커서 시 Selected 영역에만 표시 (§2.2 규칙)
- **⚠️ 패드 경계 홀드 리셋**: 손가락이 패드 경계를 넘는 순간 터치업 이벤트가 발생하여 BridgeOne 측 홀드 상태가 리셋됩니다. 클릭 홀드를 유지한 채 패드를 넘나드는 드래그는 외부 보조 버튼과 조합하여 수행합니다 (§3.2.4.3 드래그 워크플로우 참조).

#### 1.2.2 패드 직접 전환 버튼 패널 (PadSwitchButtonPanel)

직접 전환 버튼 모드에서 표시되는 하단 전환 버튼 패널:
- 표시 조건: 멀티 커서 모드 활성화 + 레이아웃 모드가 `직접 전환 버튼`으로 설정된 경우
- 위치: `TouchpadAreaWrapper` 하단 고정
- 높이: 48dp
- 구성: N개의 버튼 (pad1~padN), 각 버튼 탭 시 해당 패드로 즉시 점프
- 활성 패드 버튼: 파란색(`#2196F3`) 강조 표시
- 비활성 패드 버튼: 동일 색 반투명(`alpha = 0.4f`)
- 모서리: 패널 외곽만 둥글게(`RoundedCornerShape(8dp)`), 개별 버튼 사이는 붙어 있고 각지게 유지 — 터치패드 본체의 둥근 모서리와 시각적 통일
- **패드 커스텀 라벨 (Phase 4.8.10)**: 버튼 텍스트는 기본적으로 번호(1~N)지만, 롱프레스로 진입하는 이름 편집 팝업(BridgeOne 내장 키보드 사용, 시스템 IME 미사용)에서 지정한 이름이 있으면 번호 대신 표시된다. 라벨은 SharedPreferences에 영속화되어 앱 재시작 후에도 유지되며, 빈 이름으로 저장하면 번호로 폴백한다.

#### 1.2.3 멀티 커서 선택 상태 관리

- 초기 상태: `pad1`이 Selected 상태, 나머지 패드는 Unselected
- **레이아웃 모드 전환**: `CursorModeButton` 롱프레스로 그리드 분할 ↔ 직접 전환 버튼 토글 (Phase 4.8.4)
- **레이아웃 모드**:
  - **그리드 분할** (기본): Unselected 패드 영역 직접 터치 시 해당 패드로 전환
  - **직접 전환 버튼**: 하단 버튼 탭 시 해당 패드로 즉시 점프. 탭 1번으로 어느 패드든 바로 이동.
- **소리 감지 전환** (옵션, 설정 활성화 시): 마이크로 특정 음(솔 근방 주파수 + 볼륨 임계값 조합)을 감지하면 다음 패드로 순환 전환. 헤드폰 사용 환경에서 손 조작 최소화 가능.
- 테두리 표시: Selected 상태인 패드에만 §2.2 규칙에 따른 테두리 표시
- 커서 제어: Selected 패드를 통해 해당 커서의 이동/클릭 제어
- **⚠️ 멀티 커서 상태 공유 제약**: 두 번째 페이지에 멀티 커서 터치패드를 추가할 경우, `MultiCursorState`를 앱 수준 공유 ViewModel로 리팩토링해야 합니다. Windows 서버가 가상 커서를 전역 1세트로 관리하므로, 각 터치패드 인스턴스가 독립된 상태를 가지면 페이지 전환 시 위치 정보가 어긋납니다. 상세 제약 사항: `docs/android/technical-specification-app.md` §2.2.6.1
...
> **⚠️ 풀 와이드 페이지 예외 (Phase 4.8.1, Phase 4.9.1에서 Page 3에도 확인)**: 위 "터치패드 너비" 규칙은 터치패드 자체가 화면 분할 컬럼(Page 1)일 때 기준이다. 터치패드/포인팅 영역이 전체 화면 너비를 차지하는 페이지(Page 2, Page 3 등)는 `ControlButtonContainer`를 그 전체 너비가 아니라 **Page 1의 터치패드 컬럼 폭과 동일한 비율(화면 폭 360dp 미만 60%, 그 외 64%)로 축소하고 좌우 중앙에 정렬**한다. 버튼 개수와 무관하게 컨테이너 전체가 중앙에 위치해야 한다. 근거: 풀 와이드 컨테이너에서 버튼 폭이 `(컨테이너 폭)/5`로 계산되므로, 축소 없이 그대로 두면 버튼 폭이 과도하게 커져 다른 페이지와 시각적 일관성이 깨짐. Page 3(AbsolutePointingPad)는 구현 직후 이 규칙이 누락되어 버튼이 과도하게 넓어졌다가 동일 규칙 적용으로 수정됨(`AbsolutePointingPad.kt`).
...
- **탭 동작 (싱글 → 멀티)**: 커서 수 선택 팝업(2개/3개/4개) 표시 → 선택 즉시 활성화. 팝업 외부 터치 또는 취소 시 싱글 커서 유지.
- **탭 동작 (멀티 → 싱글)**: 팝업 없이 즉시 비활성화
- 기본 상태: 싱글 커서 모드
- 표시 조건: §1.3 버튼 구성 독립성 원칙에 따라 인스턴스별로 표시/비표시 결정 (현재 Page 1 비표시, Page 2 표시)
...
- 아이콘 매핑: VectorDrawable 사용. 낮음=`ic_slow.xml`, 보통=`ic_normal.xml`, 높음=`ic_fast.xml`
  - 커스텀: 커스텀 값보다 큰 사전 정의 값 중 가장 작은 것의 아이콘 사용 (탭 순환 연결 대상과 동일)
- 배경색:
  - 사전 정의 레벨: 각 레벨의 지정 색상
  - 커스텀: 커스텀 값보다 큰 사전 정의 값 중 가장 작은 것의 색상 사용 (탭 순환 연결 대상과 동일)
- **탭 기능**: DPI 순환
...
- **롱 프레스 기능**: DPI 세밀 조절 팝업 표시 (§1.3.2.3 참조)
- 숨김 처리: 스크롤 모드 Selected 상태에서 숨김

##### 1.3.2.3 DPI 세밀 조절 팝업 (DpiAdjustPopup)

- **트리거**: DPIControlButton 롱 프레스
- **구조**: 터치패드 영역 안에만 표시되는 반투명 오버레이 (화면 전체 아님)
- **중앙 표시**: 터치패드 영역 정중앙에 현재 DPI 배율 값을 크게 표시 (예: `1.3x`)
- **수치 조절**: 터치패드 영역 내 어디든 스와이프(상하 또는 좌우 모두 동일하게 동작)
  - 한 방향으로 드래그할수록 값 증가/감소 (스와이프 거리에 비례)
  - 조절 범위: 0.1x ~ 5.0x, 스텝: 0.1 단위, 경계 초과 시도 시 피드백:
    - 배율 텍스트 잠시 붉은색으로 변경 후 원래 색으로 복귀
    - 텍스트 좌우 shake 애니메이션
    - 진동 피드백 (햅틱)
  - 드래그 시작 기준값: 팝업 오픈 시점의 현재 DPI 배율
- **확정**: 터치패드 영역 내 탭 → 현재 표시 값으로 DPI 결정, 팝업 닫힘, 햅틱
- **취소**: 팝업 오픈 상태에서 페이지 전환 발생 → 커스텀 값 적용 없이 팝업 닫힘
- **결과 반영**: 확정된 값이 사전 정의 값(0.5/1.0/2.0) 중 하나와 일치하면 해당 DpiLevel로 저장, 그 외는 임시 커스텀 값으로 저장
- **커스텀 값 초기화 조건** (임시 값이므로 아래 상황에서 소멸):
  - 탭 순환으로 사전 정의 값으로 전환 시
  - 앱 종료 후 재시작 시
  - USB 연결 끊김 시
  - 스크롤 모드 전환 시 (스크롤 모드 진입)
...
### 1.7 모드 프리셋 버튼 (ModePresetButton)

**개요**: 여러 개의 모드 구성 스냅샷(프리셋)을 저장해두고 버튼 하나로 즉시 전환하는 기능입니다. DPI, 클릭 모드, 이동 모드, 스크롤 모드, 포인터 다이나믹스를 하나의 프리셋으로 묶어 관리합니다.

- **위치**: 터치패드 영역 (`TouchpadAreaWrapper`) 내 우측 하단 오버레이 (`Alignment.BottomEnd`, `padding(end = 8.dp, bottom = 8.dp)`)
  - `DynamicsPresetButton` (`BottomStart`)과 좌우 대칭 배치
- **크기**: 40dp × 40dp (`DynamicsPresetButton`과 동일)
- **표시 내용**: 현재 프리셋 이름 약어 또는 아이콘
- **표시 조건**: Standard 모드에서 **항상 표시** — 스크롤 모드, 커서 이동 모드 등 어떤 모드 상태와 무관
  - `DynamicsPresetButton`은 스크롤 모드에서 숨겨지지만, `ModePresetButton`은 숨기지 않음
- **탭 기능**: 다음 프리셋으로 순환 (`index = (index + 1) % MODE_PRESETS.size`)
  - 탭 시 프리셋 이름 라벨이 버튼 위에 잠시 표시 후 자동 소멸
- **롱프레스 기능**: 모드 프리셋 선택 팝업 표시 (§1.7.1 참조)
- **이벤트 소비**: `down.changes.forEach { it.consume() }` — 부모 터치패드 제스처에 전파 차단
- **Essential 모드**: 숨김 (`ControlButtonContainer`와 동일 정책)

#### 1.7.1 모드 프리셋 선택 팝업 (ModePresetPopup)

`DynamicsPresetPopup` (Phase 4.3.8 구현)과 동일한 패턴으로 구현합니다.

- **트리거**: `ModePresetButton` 롱프레스
- **구조**: 터치패드 전체 영역을 덮는 반투명 오버레이
- **블러 배경**: 팝업이 열리면 뒤쪽 콘텐츠에 blur 8dp 적용
  - 블러 상태 통합 관리: `showDpiPopup || showDynamicsPopup || showModePresetPopup`
- **그리드 탐색, 탭 확인, 스와이프 조작**: `DynamicsPresetPopup`과 동일한 2단계 구조 (그리드 선택 → 확인 단계)
- **취소 조건**: 페이지 전환 발생 시 → 프리셋 변경 없이 팝업 닫힘

#### 1.7.2 커서 수 선택 팝업 (CursorCountSelectionPopup)

`CursorModeButton` 탭 시(싱글 → 멀티 전환) 표시되는 커서 수 선택 팝업입니다.

- **트리거**: `CursorModeButton` 탭 (현재 싱글 커서 상태일 때만)
- **구조**: `CursorModeButton` 위에 표시되는 소형 팝업
- **위치 (Phase 4.8.6)**: `ControlButtonContainer`의 실제 렌더 높이를 `onGloballyPositioned`로 측정해 그 바로 아래(8dp 여백)에 앵커링한다. `ControlButtonContainer` 높이는 화면 크기에 따라 48~72dp로 가변이라 고정 오프셋으로는 겹침을 피할 수 없어 실측값을 사용한다.
- **선택지**: 가로 배열 3개 버튼 — `[2]` `[3]` `[4]`
  - 각 버튼은 커서 수를 나타내는 숫자 텍스트
  - 버튼 크기: 제어 버튼과 동일한 터치 영역 확보 (접근성)
- **배경**: 반투명 다크 배경 (`#1E1E1E`, 90% 불투명도), 라운드 코너
- **등장 애니메이션**: 버튼 위치에서 위로 슬라이드 + 페이드인 (150ms ease-out)
- **선택 동작 (Phase 4.8.8)**: 버튼 탭 → 즉시 적용하지 않고 아래 "패드별 프리셋 지정 단계"로 전환. 이 단계에서 "확인"을 눌러야 실제로 적용된다.
- **취소 동작**: 팝업 외부 터치 → 팝업 닫힘, 원래 상태 유지 (모드/개수/패드별 프리셋 모두 변경 없음)
- **퇴장 애니메이션**: 등장의 역재생(위로 슬라이드 + 페이드아웃, 120ms, Phase 4.8.6)
- **블러 배경**: 기존 팝업과 동일한 블러 상태 통합 관리에 포함
  - `showDpiPopup || showDynamicsPopup || showModePresetPopup || showCursorCountPopup`

**활성 중 모드 (Phase 4.8.6)**: 멀티 커서 활성 상태에서도 `CursorModeButton` 탭으로 이 팝업이 열린다(즉시 해제하지 않음).
- 현재 활성 커서 수에 해당하는 버튼이 연한 보라색 틴트 배경(`TouchpadColorPurple` 45% 불투명도) + 보라색 테두리로 강조 표시된다(`ModePresetPopup`의 "현재 항목 틴트" 컨벤션과 통일, 색은 `CursorModeButton`이 멀티 진입 시 쓰는 대표색과 일치).
- 우측에 빨간색 "해제" 버튼이 추가로 표시된다. 이 버튼만 예외적으로 즉시 동작한다(pending 없이 바로 싱글 커서로 복귀).
- 강조되지 않은 수를 탭해도 (비활성 상태와 동일하게) 즉시 적용되지 않고 패드별 프리셋 지정 단계로 전환된다(Phase 4.8.8).

**패드별 프리셋 지정 단계 (Phase 4.8.8)**: 개수 버튼 탭 직후 진입하는 2번째 단계. 활성/비활성 상태 모두 동일하게 거친다.
- 선택한 개수만큼 "패드 N" 행이 세로로 나열되며, 각 행에는 현재 지정된 프리셋 이름(또는 "미지정")이 표시된다.
- 행 탭 시 프리셋이 **미지정 → Standard → Precise → Fast → 미지정** 순으로 순환한다(§1.7 `MODE_PRESETS` 재사용, 신규 프리셋 정의 없음).
- 기본값: 사용자가 이 화면에서 한 번도 확정한 적이 없으면, 모든 패드가 **싱글 커서 모드에서 현재 사용 중인 프리셋**으로 시작한다. 확정 이력이 있으면 그 값이 세션 동안 유지된다(SharedPreferences 영속 없음, 앱 재시작 시 다시 싱글 커서 프리셋 추종으로 복귀).
- 미지정 패드는: 이미 활성 중이던 패드면 그 상태를 그대로 보존하고, 신규 패드이거나 애초에 비활성이었다면 싱글 커서의 현재 모드 상태를 시드로 사용한다(하위 호환).
- 하단 "확인" 버튼(가운데 정렬) 탭 → 선택한 개수 + 패드별 프리셋 매핑이 `MultiCursorController`에 반영되며 팝업이 닫힌다.
- 외부 터치 시 이 단계에서도 전체 취소(팝업 닫힘, 아무 것도 변경되지 않음).
...
적용 대상:
- 싱글 커서 모드: `Touchpad1Area` (pad1) 테두리
- 멀티 커서 모드: pad1~padN 중 Selected 패드에만 테두리 표시, Unselected 패드는 테두리 없음
...
1. **싱글 → 멀티 (활성화)**:
   ...
   2. 커서 수 선택 팝업 표시 (2개 / 3개 / 4개)
   3. 사용자가 커서 수(N) 선택 → 즉시 활성화 진행
   4. 단일 → N개 영역 분할 또는 직접 전환 버튼 표시, 버튼 텍스트/아이콘/배경 갱신(배경 도형색=`#B552F6`), 300ms 분할 애니메이션(ease-out)
   5. 적용: 애니메이션이 시작되는 순간부터 작업을 시작해 애니메이션 종료 시점에는 새로운 커서 모드 반영
   - 팝업 외부 터치 또는 취소 시: 아무 동작 없이 싱글 커서 유지
2. **멀티 → 싱글 (비활성화)**:
   1. 버튼 터치 감지: 클릭 애니메이션 및 햅틱
   2. 팝업 없이 즉시 비활성화
   3. 분할 → 단일 복귀, 버튼 텍스트/아이콘/배경 갱신(배경 도형색=`#2196F3`), 300ms 통합 애니메이션(ease-in)
   4. 적용: 애니메이션이 시작되는 순간부터 작업을 시작해 애니메이션 종료 시점에는 새로운 커서 모드 반영

> **양방향 전환 기술 명세**: 멀티 커서 활성화와 싱글 커서 복원의 통신 프로토콜, 상태 동기화, 메모리 관리 등은 `docs/android/technical-specification-app.md` §2.2.6(멀티 커서 알고리즘 명세) 참조

##### 3.2.4.1 터치패드 영역 분할 및 전환

**영역 분할 시각적 표현:**
1. 전환 감지: 활성 패드 이외의 영역(그리드 분할) 또는 전환 버튼(직접 전환 버튼 모드) 탭 시 해당 패드로 즉시 전환 (탭 시 햅틱 피드백)
2. 상태 표시: 활성 영역은 §2.2 규칙에 따른 테두리 표시, 비활성 영역은 테두리 없음 + 점선 테두리 + dim 오버레이(패드 번호 또는 커스텀 라벨 포함, Phase 4.8.10). 비활성 셀 롱프레스로 이름 편집 팝업 진입 가능(활성 셀은 터치패드 제스처와 충돌해 미지원)
3. **전환 효과 (Phase 4.8.6)**:
   - **그리드 분할 모드**: 파란색 하이라이트 테두리가 이전 셀에서 새 셀로 250ms 슬라이드 이동, 도착 시 펄스(0.1초, 80% 불투명도에서 소멸)로 마무리. 비활성 셀의 dim은 활성 전환에 맞춰 페이드
   - **직접 전환 버튼 모드**: 터치패드 본체가 페이저처럼 방향성 있게 슬라이드(250ms) — 인덱스 증가 시 왼쪽으로 밀리며 새 패드가 오른쪽에서 들어옴. 하단 전환 버튼 패널의 활성 하이라이트도 동일 방향으로 슬라이드(200ms)

**레이아웃 모드** (설정에서 선택):
- **그리드 분할** (기본): 터치패드를 N개 셀로 분할. 비활성 셀 탭 시 즉시 전환. N에 따른 레이아웃 자동 결정.
- **직접 전환 버튼**: 터치패드 전체 면적 유지. 하단 N개 버튼으로 직접 점프. 탭 1번으로 어느 패드든 이동 가능.

**패드 전환 트리거 방법** (추가 옵션):
- **소리 감지 전환** (옵션): 설정에서 활성화 시 마이크 상시 모니터링. 특정 음(솔 근방 주파수 + 볼륨 임계값 조합) 감지 시 다음 패드로 순환 전환. 헤드폰 착용 환경에서 컴퓨터 오디오 차단 상태로 사용 가능. 감도는 설정에서 조정.
...
2. 작업 흐름: 왼쪽 터치패드로 코드 편집기 커서 위치 조정 → 패드 전환(탭) → 오른쪽 터치패드로 브라우저 커서 위치 조정 → 반복
3. 사용자 경험: 매끄러운 멀티태스킹으로 작업 효율성 대폭 향상

**드래그를 수반하는 워크플로우 (텍스트 선택, 파일 이동 등):**

BridgeOne의 터치패드는 패드 경계를 넘는 순간 터치업(손가락 뗌) 이벤트가 발생하여 홀드 상태가 리셋됩니다. 따라서 한 패드에서 클릭을 홀드한 채 다른 패드로 이어서 드래그하는 동작은 BridgeOne 단독으로는 불가능합니다.

이 제약은 사용자가 **외부 좌클릭/우클릭 보조 버튼**을 함께 활용하는 방식으로 해소됩니다:

1. **텍스트/영역 선택 예시:**
   - 외부 좌클릭 버튼을 누른 채 왼쪽 패드에서 드래그 시작 → 선택 시작점에서 커서 이동
   - 버튼을 계속 누른 상태에서 오른쪽 패드를 터치 → 패드 전환(텔레포트) → 선택 끝점으로 커서 이동
   - 외부 버튼 해제 → 선택 완료
2. **파일 드래그 앤 드롭 예시:**
   - 파일 위에 커서 위치 → 외부 좌클릭 버튼 누름 + 왼쪽 패드 드래그로 파일 홀드
   - 버튼 유지한 채 오른쪽 패드 터치로 패드 전환 → 목적지로 커서 이동
   - 외부 버튼 해제 → 드롭 완료

> **핵심**: BridgeOne은 커서 위치 제어를 담당하고, 클릭 홀드 상태 유지는 외부 보조 버튼이 독립적으로 관리합니다. 패드 경계에서 BridgeOne의 홀드가 리셋되더라도 외부 버튼이 눌린 상태를 계속 유지하므로, OS 입장에서는 드래그가 끊기지 않습니다.

> **기술적 세부사항**: 멀티 커서 모드 전환 및 싱글 커서 복원의 통신 프로토콜, 성능 요구사항, 오류 처리 등의 구현 명세는 `docs/android/technical-specification-app.md` §2.2.6(멀티 커서 알고리즘 명세) 참조

##### 3.2.4.4 멀티 커서와 기능 모드 상호작용 규칙

**기능 모드는 패드별(per-pad) 독립입니다.** 각 패드는 자신만의 클릭 모드, 이동 모드, 스크롤 모드, DPI 설정을 독립적으로 보유합니다. `ControlButtonContainer`는 1세트만 존재하지만, 활성 패드가 전환될 때 버튼들이 새 패드의 상태를 즉시 반영합니다.

**패드 전환 시 UI 동작:**

| 기능 모드 | 패드 전환 시 시각적 변화 | 동작 |
|----------|----------------------|------|
| 클릭 모드 (좌/우) | ClickModeButton이 새 활성 패드의 설정으로 즉시 업데이트 | 각 패드 독립 설정 유지 |
| 이동 모드 (자유/직각) | MoveModeButton이 새 활성 패드의 설정으로 즉시 업데이트 | 각 패드 독립 설정 유지 |
| 스크롤 모드 | ScrollModeButton이 새 활성 패드의 설정으로 즉시 업데이트 | 스크롤 가이드라인도 패드별 독립 |
| DPI | DPIControlButton이 새 활성 패드의 설정으로 즉시 업데이트 | 각 패드 독립 감도 적용 |

**`ControlButtonContainer` 위치 고정 원칙:**
`ControlButtonContainer`는 멀티 커서 활성화 여부나 활성 패드 전환과 무관하게, **원본 전체 터치패드(`TouchpadWrapper`) 기준 좌표에 항상 고정**됩니다. 그리드 분할로 패드가 나뉘어도, 활성 패드가 pad2로 변경되어도 버튼 컨테이너는 이동하지 않습니다. 이를 통해 패드 전환 전후에 버튼 위치가 일관되게 유지됩니다.

**DeltaCalculator 리셋:**
- 패드 전환 시 DeltaCalculator의 previousX/Y가 초기화되어 터치 점프를 방지합니다
- 상세 명세: `docs/android/technical-specification-app.md` §2.2.6.7

**멀티 커서 초기 위치:**
- Android 앱은 PC 화면 크기를 모르므로 pad1~padN 초기 위치는 Windows 서버가 결정합니다
- pad1 = 현재 실제 커서 위치, pad2~padN = 주 모니터 중앙 기준 분산 배치 (기본값)
- 상세 명세: `docs/android/technical-specification-app.md` §2.2.6.9
...
- 시스템 응답:
  1) 버튼 클릭 애니메이션, 햅틱
  2) 순환 (커스텀 없을 때): 높음 → 낮음 → 보통 → 높음
  3) 순환 (커스텀 있을 때): 커스텀 → 높음 → 낮음 → 보통 → 높음 (커스텀 소멸 후 사전 정의 순환으로 진입)
  4) 적용: 새로운 DPI 즉시 반영, 다음 이동부터 적용

##### 3.2.5.2 DPI 세밀 조절 팝업
- 사용자 액션: DPI 버튼 롱 프레스
- 시스템 응답:
  1) 햅틱 (롱 프레스 감지)
  2) 터치패드 영역 내 반투명 오버레이 표시, 터치패드 정중앙에 현재 DPI 배율 크게 표시
  3) 터치패드 내 스와이프 → 드래그 거리에 비례하여 배율 값 실시간 증감
  4) 터치패드 내 탭 → 현재 배율로 DPI 확정, 팝업 닫힘, 햅틱
  5) 페이지 전환 발생 시 → 커스텀 값 적용 없이 팝업 취소
  6) 버튼 텍스트 업데이트: 커스텀이면 `1.3x` 형식, 사전 정의 값과 일치하면 레벨명으로 표시
