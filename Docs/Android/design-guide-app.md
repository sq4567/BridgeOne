...
- Enabled/Disabled: 입력 가능 상태. 탭/제스처 허용 여부. (예: 작업 중 버튼 Disabled.)
  - Disabled 상태 사용 시나리오: 작업 진행 중, 권한 부족, 연결 문제, 시스템 제약 등
  - Disabled 상태 전환 조건: 작업 시작 시 자동 Disabled, 작업 완료/취소 시 자동 Enabled 복원
- TransportState: NoTransport | UsbOpening | UsbReady - USB 연결 상태를 나타냄.
- AppState: WaitingForConnection | Essential | Standard - 앱 전체 상태로 화면 표시를 결정.
  - WaitingForConnection: USB 장치 연결 대기 상태
  - Essential: Windows 서버 미연결 상태 (기본 기능만 사용 가능)
  - Standard: Windows 서버 연결 상태 (모든 기능 사용 가능)
- 활성 매크로 (Active Macro): 실제 키보드/마우스 조작이 수반되는 매크로. 매크로 실행 중 다른 조작 시 오작동 가능성이 높아 페이지 전체 입력 차단이 필요.
- 비활성 매크로 (Inactive Macro): SendMessage, PostMessage 등을 활용해 특정 프로세스에 키보드/마우스 메시지를 전송하는 매크로. 매크로 실행 중 다른 조작과 충돌 가능성이 낮아 개별 컴포넌트만 선택적 비활성화.
...
3. Animated Splash Screen 시작 (2.5초, 6단계 애니메이션 시퀀스) + 병렬 초기 감지:
   - 상세 설계: `splash-screen-guide-app.md` 참조
