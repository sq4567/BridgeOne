...
- 입력 보정:
  - 이동 시작 판정 및 초기 이동량 반영은 데드존 보상 로직을 따름(`Docs/Android/technical-guide-app.md` §2.1.3 참조).
...
- 상세한 직각 이동 알고리즘은 `Docs/Android/technical-guide-app.md` §2.1.2(직각 이동 알고리즘 명세) 참조
...
- 가이드라인 상세 동작은 `Docs/Android/technical-guide-app.md` §2.1.4(스크롤 가이드라인 알고리즘 명세) 참조
...
- 무한 스크롤 가이드라인 상세 동작은 `Docs/Android/technical-guide-app.md` §2.1.4(스크롤 가이드라인 알고리즘 명세) 참조
...
> **양방향 전환 기술 명세**: 멀티 커서 활성화와 싱글 커서 복원의 통신 프로토콜, 상태 동기화, 메모리 관리 등은 `Docs/Android/technical-guide-app.md` §2.1.5(멀티 커서 알고리즘 명세) 참조
...
> **기술적 세부사항**: 멀티 커서 모드 전환 및 싱글 커서 복원의 통신 프로토콜, 성능 요구사항, 오류 처리 등의 구현 명세는 `Docs/Android/technical-guide-app.md` §2.1.5(멀티 커서 알고리즘 명세) 참조

#### 3.2.4.1 특수 모드 플로우: Essential/Standard 모드

> 📋 **기술 구현 세부사항**: Essential/Standard 모드의 기능 제한, 프로토콜 제한사항, 상태 전이 로직 등의 기술적 세부사항은 별도 기술 문서 `Docs/Android/technical-guide-app.md` §7.6(Essential/Standard 모드 관리)를 참조하세요.
...
> 📋 **기술 구현 세부사항**: 터치패드 컴포넌트의 모든 상세 알고리즘과 구현 로직은 별도 기술 문서 `Docs/Android/technical-guide-app.md` §2(터치패드 핵심 알고리즘)를 참조하세요.

**주요 알고리즘 요약:**
- **자유 이동**: 좌표 변환, DPI 정규화, 8바이트 프레임 생성
- **직각 이동**: 축 결정 알고리즘, 각도 기반 8분할 판정, 축 고정 유지
- **데드존 보상**: 상태 머신 기반 누적/보상 메커니즘, 클릭 정확도 향상
- **스크롤 가이드라인**: 속도 동기화, 관성 스크롤, 시각적 방향성 표시

---

## 5. 상수/임계값 (Standardized Constants)

> 📋 **통합 상수 관리**: 터치패드 컴포넌트의 모든 상수와 임계값은 별도 기술 문서 `Docs/Android/technical-guide-app.md` §4(상수 및 임계값 통합)를 참조하세요.

**주요 상수 카테고리:**
- **레이아웃/UI**: 크기, 비율, 간격, 모서리 반지름
- **색상**: 모드별 대표 색상, 투명도, 상태별 색상
- **입력 인식**: 데드존, 클릭 인식, 축 결정 임계값
- **동작**: 민감도, 스크롤, 가이드라인 관련 상수
- **애니메이션**: 지속시간, 이징, 전환 효과
- **통신**: 전송 주기, 프로토콜 크기, 타임아웃

**상수 관리 원칙:**
- 중앙 집중식 관리로 일관성 보장
- 컴파일 타임 상수 활용으로 성능 최적화
- 명명된 상수 사용으로 매직 넘버 방지
