...
> **핵심**: Essential 모드에서는 필수 마우스 기능(이동, 3버튼)과 키보드 기능(Del, Esc, Enter, F1-F12, 방향키)을 지원하며, 휠/고급 기능은 비활성화됩니다. ESP32-S3는 복합 USB 장치로서 구성되지만, Essential 모드에서는 HID 인터페이스(Boot Protocol)만 사용하여 마우스와 키보드 기능을 제공하고 Vendor-specific Interface는 비활성화됩니다.

## 용어집/정의

- Selected/Unselected: 선택 상태. 시각 강조 여부(예: 방향 하이라이트). 입력 가능과 구분.
- Enabled/Disabled: 입력 가능 상태. Essential 단계에서는 많은 요소가 Disabled.
- Essential/Standard: 페이지 운용 상태. 본 문서는 Essential 전용.
- USB Composite Device: ESP32-S3가 PC에 제공하는 복합 USB 장치. 단일 USB 연결을 통해 여러 인터페이스를 동시에 제공.
- HID Interface: Human Interface Device 인터페이스. Essential 모드에서는 Boot Protocol만 사용하여 기본 마우스/키보드 기능을 제공.
- Vendor CDC Interface: Custom Communication Device Class 인터페이스. Essential 모드에서는 비활성화되며 Windows 서버 연결 시에만 사용.
- 상태 용어 사용 원칙(금칙어 포함): "활성/비활성" 금지. Selected/Unselected, Enabled/Disabled로 표기 [[memory:5809234]].

## 1. 개요

이 문서는 Essential 모드 전용 페이지의 **UI 레이아웃과 스타일**을 정의합니다. 본 페이지는 PC가 Windows 서버와 연결되지 않은 모든 상태(BIOS/UEFI 등 OS 부팅 전 단계 포함)에서 표준 HID 입력 장치로 동작할 수 있도록 최소한의 UI를 제공합니다. 상태 용어는 선택 여부는 `Selected/Unselected`, 입력 가능 여부는 `Enabled/Disabled`를 사용합니다 [[memory:5809234]].
