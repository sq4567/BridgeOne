...
> **핵심**: 마인크래프트 게임플레이에 최적화된 D패드와 게임 버튼을 제공하는 페이지입니다. ESP32-S3는 복합 USB 장치로서 HID 인터페이스(마우스/키보드 입력)와 Vendor CDC 인터페이스(Windows 서버 통신)를 동시에 제공합니다.
...
- HID Interface: Human Interface Device 인터페이스. 마우스/키보드 입력을 위한 표준 USB 클래스. 게임 특화 입력 처리를 지원.
- Vendor CDC Interface: Custom Communication Device Class 인터페이스. Windows 서버와의 양방향 통신을 위한 커스텀 시리얼 포트.
