...
- HID Interface: Human Interface Device 인터페이스. 마우스/키보드 입력을 위한 표준 USB 클래스. Boot Protocol과 Report Protocol 지원.
- Vendor-specific Interface: Custom Communication Device Class 인터페이스. Windows 서버와의 양방향 통신을 위한 커스텀 시리얼 포트.
...
권장 기본: `Macro 1`, `Macro 2`, `Macro 3` — 사전 정의된 실행 전용 매크로.
...
- 아이콘: VectorDrawable 사용(예: `ic_play.xml`). `ic_stop.xml`은 별도 중지 액션이 있을 때만 사용.
