...
### `PRD.md`: 제품 요구사항 정의서
프로젝트의 목표, 해결하려는 문제, 사용자 프로필, 핵심 기능 요구사항을 정의하는 **최상위 기획 문서**입니다.
...
### `integrated-user-flow-guide.md`: Android 앱, ESP32-S3 동글, Windows 서버 간의 전체 사용자 경험을 통합적으로 설계한 사용자 플로우 가이드
...
### `document-review-plan-checklist.md`: 프로젝트 문서의 품질 보증을 위한 체크리스트 및 검토 프로세스 가이드

**주요 내용:**
- 문서별 검토 기준 및 체크리스트
- 검토자 역할 및 책임 정의
- 문서 업데이트 및 버전 관리 프로세스
- 품질 기준 및 승인 절차
...
### `Board/usb-hid-bridge-architecture.md`: Android 앱, ESP32-S3 동글, PC 간의 하드웨어 연결 및 데이터 흐름 아키텍처를 상세히 설명하는 USB HID 브릿지 아키텍처

**주요 내용:**
- 3-tier 아키텍처 설계 (Android ↔ ESP32-S3 ↔ PC)
- USB Serial/HID 프로토콜 변환 메커니즘
- 하드웨어 연결 및 신호 흐름
- 프로토콜 최적화 전략
...
### `splash-screen-guide-server.md`: Windows 서버 애니메이션 스플래시 스크린

**주요 내용:**
- **6단계 애니메이션 시퀀스**: 초기 검은 화면 → 브릿지 등장 → 별 회전 → 확장 → 텍스트 등장 (총 2.5초)
- **Windows 기술 구현**: WPF Storyboard 기반 애니메이션
- **시스템 통합**: Windows 서비스와의 연동, 트레이 아이콘 연동
- **크로스플랫폼 일관성**: Android 앱과 동일한 타이밍과 시각적 일관성 보장
...
- **`Windows/splash-screen-guide-server.md`**: Windows 서버 프로그램의 브랜드 아이덴티티를 반영한 6단계 애니메이션 스플래시 스크린 구현 가이드
  - **Windows**: WPF Storyboard 기반 애니메이션 구현
  - **시스템 통합**: Windows 서비스와의 연동, 트레이 아이콘 연동
  - **크로스플랫폼 일관성**: Android 앱과 동일한 타이밍과 시각적 일관성 보장
...
#### 6.1.1 CP2102 USB-Serial 어댑터 (Silicon Labs)
**출처**: [Silicon Labs CP2102 공식 페이지](https://www.silabs.com/interface/usb-bridges/classic/device.cp2102?tab=techdocs)

**필수 문서 (6개)**:
- **AN197**: Serial Communications Guide for the CP210x (v1.1): UART 통신 기본 원리
- **AN809**: Integrating the CP210X Virtual COM Port Driver into the Android Platform (v0.2): Android 플랫폼 통합
- **AN571**: CP210x Virtual COM Port Interface (v0.4): VCP 인터페이스 이해
- **AN978**: CP210x USB-to-UART API Specification (v1.5): 프로그래밍 인터페이스
- **AN220**: USB Driver Customization (v1.1): 드라이버 커스터마이징
- **AN335**: USB Driver Installation Methods (v0.4): 드라이버 설치 방법

**BridgeOne 활용**: Android 앱에서 CP2102 인식, UART 통신 구현, Windows 드라이버 호환성

#### 6.1.2 ESP32-S3 공식 문서 (Espressif)
...
- **TinyUSB 라이브러리 문서**: HID Boot Protocol, 복합 장치 구성

**BridgeOne 활용**: ESP32-S3 펌웨어 개발, UART 통신, USB HID 마우스 구현

#### 6.1.3 Geekble nano ESP32-S3 보드 매뉴얼
**출처**: [Geekble nano ESP32-S3 보드 매뉴얼](https://github.com/Geekble-nano-ESP32S3/)

**필수 문서**:
- **보드 매뉴얼**: 핀 배치, 전력 요구사항, 하드웨어 연결 방법
- **하드웨어 디자인 가이드**: PCB 설계, 신호 무결성

**BridgeOne 활용**: CP2102와의 정확한 핀 연결, 전원 공급 방법, 보드별 특수 기능

### 6.2 Android 개발 관련 문서

#### 6.2.1 Android USB OTG 개발 가이드
...
**BridgeOne 활용**: CP2102 인식, OTG Y 케이블 호환성, 연결 안정성 확보

#### 6.2.2 USB-Serial-for-Android 라이브러리
**출처**: [USB-Serial-for-Android GitHub](https://github.com/mik3y/usb-serial-for-android)

**필수 문서**:
- **라이브러리 문서**: CP2102 전용 통신 구현
- **API 참조**: 양방향 데이터 전송 방법
- **예제 코드**: 실제 구현 사례

**BridgeOne 활용**: CP2102와의 안정적인 UART 통신, 8바이트 프레임 전송/수신

#### 6.2.3 Android 접근성 개발 가이드
...
### 6.3 Windows 개발 관련 문서

#### 6.3.1 Windows HID API 문서
...
**BridgeOne 활용**: CP2102와 ESP32-S3 간 UART 통신 최적화

### 6.5 개발 단계별 우선순위

#### Phase 1 (Android 앱 기본 완성): 즉시 필요
1. **CP2102 관련 문서** (완료)
2. **ESP32-S3 공식 문서**
3. **Geekble nano 보드 매뉴얼**
4. **Android USB OTG 개발 가이드**
5. **USB-Serial-for-Android 라이브러리**

#### Phase 2 (핵심 입력 시스템): 개발 진행 중
1. **TinyUSB 라이브러리 문서**
2. **Android 터치패드 개발 가이드**
3. **USB HID 표준 문서**

#### Phase 3 (Windows 서버 연동): 고급 기능
1. **Windows HID API 문서**
2. **C# .NET USB 통신 자료**
3. **WPF UI 개발 가이드**

#### Phase 4 (안정성 및 품질): 최종 단계
1. **접근성 개발 가이드**
2. **성능 최적화 문서**
3. **테스트 및 검증 가이드**
...
BridgeOne 프로젝트의 각 플랫폼별 개발 환경 설정을 위한 워크스페이스 규칙 생성기 문서들입니다.

#### 7.1 Android 개발 환경
- **`android-workspace-rule-generator.md`**: Android 앱 개발을 위한 Cursor IDE 워크스페이스 규칙 생성 가이드
  - Android 개발 환경 설정 및 최적화
  - Jetpack Compose 개발 패턴 및 규칙
  - USB 통신 및 터치패드 컴포넌트 개발 가이드
  - 코드 스타일 및 아키텍처 패턴 정의

#### 7.2 ESP32-S3 펌웨어 개발 환경
- **`board-workspace-rule-generator.md`**: ESP32-S3 펌웨어 개발을 위한 Cursor IDE 워크스페이스 규칙 생성 가이드
  - ESP-IDF 개발 환경 설정
  - UART 통신 및 USB HID 구현 패턴
  - 하드웨어 인터페이스 개발 규칙
  - 펌웨어 업데이트 및 디버깅 가이드

#### 7.3 Windows 서버 개발 환경
- **`windows-workspace-rule-generator.md`**: Windows 서버 프로그램 개발을 위한 Cursor IDE 워크스페이스 규칙 생성 가이드
  - C# .NET 개발 환경 설정
  - WPF GUI 개발 패턴 및 규칙
  - Windows 서비스 및 HID API 활용 가이드
  - 멀티 커서 및 매크로 시스템 구현 규칙

**주요 목적:**
- 각 플랫폼별 개발 환경의 일관성 보장
- 개발자 온보딩 시간 단축
- 코드 품질 및 아키텍처 일관성 유지
- 플랫폼 간 통합 개발 프로세스 최적화
