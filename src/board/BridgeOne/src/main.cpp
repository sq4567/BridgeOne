/**
 * @file main.cpp
 * @brief Phase 1.2.4.1: ESP32-S3 테스트 펌웨어 작성
 * @details HID 출력 및 CDC 명령 처리를 단독으로 테스트할 수 있는 펌웨어
 * @note Phase 1.2.2.3 코드 기반 위에 테스트 모드 추가
 * @note Arduino ESP32 USB HID Mouse/Keyboard API 사용 (Context7 공식 문서 기반)
 * @note 공식 문서 출처:
 ...
 *       - USBHIDMouse, USBHIDKeyboard, Serial 객체 사용
 * @note 테스트 명령:
 *       - 'm': 마우스 테스트 (우측 50픽셀 이동)
 *       - 'c': 클릭 테스트 (좌클릭 down → 100ms → up)
 *       - 'k': 키보드 테스트 ('Hello' 입력)
 *       - 's': 스크롤 테스트 (휠 ±3)
 *       - 'p': CDC PING 명령 전송
 *       - 'a': 자동 테스트 시퀀스 실행
 * @note 자동 테스트: setup() 완료 5초 후 자동으로 모든 테스트 순차 실행
 */
...
// ============================================================================
// Phase 1.2.4.1: 테스트 모드 전역 변수
// ============================================================================

// 테스트 모드 플래그
bool g_testModeEnabled = false;         // 테스트 모드 활성화 여부
bool g_autoTestCompleted = false;       // 자동 테스트 완료 여부
uint32_t g_setupCompleteTime = 0;       // setup() 완료 시간 (ms)

// 테스트 명령 Enum
enum TestCommand : uint8_t {
  TEST_CMD_NONE = 0x00,           // 명령 없음
  TEST_CMD_MOUSE = 'm',           // 'm': 마우스 테스트
  TEST_CMD_CLICK = 'c',           // 'c': 클릭 테스트
  TEST_CMD_KEYBOARD = 'k',        // 'k': 키보드 테스트
  TEST_CMD_SCROLL = 's',          // 's': 스크롤 테스트
  TEST_CMD_PING = 'p',            // 'p': CDC PING 명령
  TEST_CMD_AUTO = 'a'             // 'a': 자동 테스트 시퀀스
};
...
// ============================================================================
// Phase 1.2.4.1: 테스트 모드 함수 구현
// ============================================================================

/**
 * @brief 테스트 모드 활성화
 * 
 * Phase 1.2.4.1에서 구현: 테스트 모드를 활성화하고 시리얼 모니터 명령 입력을 허용합니다.
 * 테스트 모드가 활성화되면 HID 출력 및 CDC 명령을 단독으로 테스트할 수 있습니다.
 * 
 * @note 참조: @docs/development-plan-checklist.md §1.2.4.1
 * @note Arduino ESP32 USB Serial API 사용 (Context7 공식 문서 기반)
 * @note 공식 문서 출처: https://github.com/espressif/arduino-esp32
 */
void enableTestMode() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println("  Phase 1.2.4.1: Test Mode Enabled");
  Serial.println("========================================");
  Serial.println();
  Serial.println("[TEST MODE] Available Commands:");
  Serial.println("  'm': Mouse Test (Move right 50 pixels)");
  Serial.println("  'c': Click Test (Left click down → 100ms → up)");
  Serial.println("  'k': Keyboard Test (Type 'Hello')");
  Serial.println("  's': Scroll Test (Wheel ±3)");
  Serial.println("  'p': CDC PING Command");
 ...
 * @brief 마우스 이동 테스트
 * 
 * 우측으로 50픽셀 이동하는 마우스 테스트를 실행합니다.
 * 
 * @note Arduino USBHIDMouse API 사용 (Context7 공식 문서 기반)
 * @note 공식 문서 출처: https://github.com/espressif/arduino-esp32
 */
void runMouseTest() {
  Serial.println("[MOUSE TEST] Moving right 50 pixels...");
  
  // 우측으로 50픽셀 이동 (5번 반복, 각 10픽셀)
  for (int i = 0; i < 5; i++) {
    Mouse.move(10, 0, 0, 0);  // x=10, y=0, wheel=0, hWheel=0
    delay(20);
  }
  
  Serial.println("[MOUSE TEST] ✓ Mouse movement completed");
}

/**
 * @brief 클릭 테스트
 * 
 * 좌클릭 down → 100ms 대기 → up 시퀀스를 실행합니다.
 * 
 * @note Arduino USBHIDMouse API 사용 (Context7 공식 문서 기반)
 * @note 공식 문서 출처: https://github.com/espressif/arduino-esp32
 */
void runClickTest() {
  Serial.println("[CLICK TEST] Left click down...");
  
  // 좌클릭 버튼 누르기
  Mouse.press(MOUSE_LEFT);
  
  Serial.println("[CLICK TEST] Waiting 100ms...");
  delay(100);
  
  // 좌클릭 버튼 떼기
  Mouse.release(MOUSE_LEFT);
  
  Serial.println("[CLICK TEST] ✓ Click test completed");
}
 ...
 * 'Hello' 문자열을 입력합니다.
 * 
 * @note Arduino USBHIDKeyboard API 사용 (Context7 공식 문서 기반)
 * @note 공식 문서 출처: https://github.com/espressif/arduino-esp32
 */
void runKeyboardTest() {
  Serial.println("[KEYBOARD TEST] Typing 'Hello'...");
  
  // 'Hello' 문자열 입력
  Keyboard.print("Hello");
  
  Serial.println("[KEYBOARD TEST] ✓ Keyboard test completed");
}
 ...
 */
void runScrollTest() {
  Serial.println("[SCROLL TEST] Scroll up (+3)...");
  
  // 휠 위로 스크롤 (양수)
  Mouse.move(0, 0, 3, 0);
  delay(500);
  
  Serial.println("[SCROLL TEST] Scroll down (-3)...");
  
  // 휠 아래로 스크롤 (음수)
  Mouse.move(0, 0, -3, 0);
  delay(500);
  
  Serial.println("[SCROLL TEST] ✓ Scroll test completed");
}

/**
 * @brief CDC PING 명령 테스트
 * 
 * Windows 서버로 PING 명령을 전송합니다.
 * 
 * @note ArduinoJson API 사용 (Context7 공식 문서 기반)
 * @note 공식 문서 출처: https://github.com/bblanchon/arduinojson
 */
void runPingTest() {
  Serial.println("[PING TEST] Sending PING command to Windows...");
  
  // JSON 문서 생성
  StaticJsonDocument<128> pingDoc;
  pingDoc["cmd"] = CMD_PING;
  pingDoc["id"] = millis();  // 메시지 ID로 현재 시간 사용
  
  // JSON 메시지 전송
  if (sendJsonMessage(pingDoc)) {
    Serial.println("[PING TEST] ✓ PING command sent successfully");
    Serial.println("[PING TEST] Waiting for response (3 seconds)...");
    ...
    while (millis() - startTime < 3000) {
      if (Serial.available() > 0) {
        ...
        if (receiveJsonMessage(responseDoc)) {
          responseReceived = true;
          
          Serial.println("[PING TEST] ✓ Response received:");
    ...
    if (!responseReceived) {
      Serial.println("[PING TEST] ✗ No response received (timeout)");
    }
  } else {
    Serial.println("[PING TEST] ✗ Failed to send PING command");
  }
}
...
void runAutoTestSequence() {
  Serial.println();
  Serial.println("========================================");
  Serial.println("  Auto Test Sequence Started");
  Serial.println("========================================");
  Serial.println();
  
  // 테스트 1: 마우스 이동
  Serial.println("[AUTO TEST] Running Mouse Test (1/5)...");
  runMouseTest();
  delay(2000);
  
  // 테스트 2: 클릭
  Serial.println("[AUTO TEST] Running Click Test (2/5)...");
  runClickTest();
  delay(2000);
  
  // 테스트 3: 키보드
  Serial.println("[AUTO TEST] Running Keyboard Test (3/5)...");
  runKeyboardTest();
  delay(2000);
  
  // 테스트 4: 스크롤
  Serial.println("[AUTO TEST] Running Scroll Test (4/5)...");
  runScrollTest();
  delay(2000);
  
  // 테스트 5: CDC PING
  Serial.println("[AUTO TEST] Running PING Test (5/5)...");
  runPingTest();
  delay(2000);
  
  Serial.println();
  Serial.println("========================================");
  Serial.println("  Auto Test Sequence Completed");
  Serial.println("========================================");
  Serial.println();
  
  g_autoTestCompleted = true;
}
...
void setup() {
  ...
  // ============================================================================
  // Phase 1.2.4.1: 테스트 모드 초기화
  // ============================================================================
  
  Serial.println("[Phase 1.2.4.1] Test Mode Initialization:");
  Serial.println("  ✓ Test mode functions implemented:");
  Serial.println("    - runMouseTest(): Move right 50 pixels");
  Serial.println("    - runClickTest(): Left click down → 100ms → up");
  Serial.println("    - runKeyboardTest(): Type 'Hello'");
  Serial.println("    - runScrollTest(): Wheel ±3");
  Serial.println("    - runPingTest(): Send CDC PING command");
  Serial.println("    - runAutoTestSequence(): Run all tests sequentially");
  Serial.println("  ✓ Test commands available via Serial Monitor:");
  Serial.println("    - 'm': Manual mouse test");
  Serial.println("    - 'c': Manual click test");
  Serial.println("    - 'k': Manual keyboard test");
  Serial.println("    - 's': Manual scroll test");
  Serial.println("    - 'p': Manual CDC PING test");
  Serial.println("    - 'a': Run auto test sequence");
  Serial.println("  ⓘ Auto test will start 5 seconds after setup() completes");
  Serial.println("========================================");
  Serial.println();
  ...
  // setup() 완료 시간 기록
  g_setupCompleteTime = millis();
}

/**
 * @brief 메인 루프 (Phase 1.2.4.1: 테스트 모드 명령 처리)
 * 
 * Phase 1.2.4.1에서 업데이트: 테스트 모드 활성화 시 시리얼 모니터에서 명령 입력을 받아 처리합니다.
 * 
 * FreeRTOS를 사용하므로 기존 로직은 별도 태스크에서 실행됩니다.
 * loop() 함수는 테스트 모드 명령 처리와 자동 테스트 시퀀스 실행에만 사용됩니다.
 * 
 * @note 참조: @docs/development-plan-checklist.md §1.2.4.1
 * @note Arduino ESP32 USB Serial API 사용 (Context7 공식 문서 기반)
 * @note 공식 문서 출처: https://github.com/espressif/arduino-esp32
...
void loop() {
  // ============================================================================
  // Phase 1.2.4.1: 자동 테스트 시퀀스 실행 (setup() 완료 5초 후)
  // ============================================================================
  
  if (g_testModeEnabled && !g_autoTestCompleted) {
    uint32_t elapsed = millis() - g_setupCompleteTime;
    
    // setup() 완료 5초 후 자동 테스트 시퀀스 실행
    if (elapsed >= 5000) {
      runAutoTestSequence();
    }
  }
  ...
  // ============================================================================
  // Phase 1.2.4.1: 시리얼 모니터 명령 처리
  // ============================================================================
  
  if (g_testModeEnabled && Serial.available() > 0) {
    ...
    switch (cmd) {
      case TEST_CMD_MOUSE:
        Serial.println();
        Serial.println("[MANUAL TEST] Mouse Test");
        runMouseTest();
        Serial.println();
        break;
      ...
      case TEST_CMD_CLICK:
        Serial.println();
        Serial.println("[MANUAL TEST] Click Test");
        runClickTest();
        Serial.println();
        break;
      
      case TEST_CMD_KEYBOARD:
        Serial.println();
        Serial.println("[MANUAL TEST] Keyboard Test");
        runKeyboardTest();
        Serial.println();
        break;
      
      case TEST_CMD_SCROLL:
        Serial.println();
        Serial.println("[MANUAL TEST] Scroll Test");
        runScrollTest();
        Serial.println();
        break;
      ...
      case TEST_CMD_PING:
        Serial.println();
        Serial.println("[MANUAL TEST] PING Test");
        runPingTest();
        Serial.println();
        break;
      ...
      case TEST_CMD_AUTO:
        Serial.println();
        Serial.println("[MANUAL TEST] Auto Test Sequence");
        runAutoTestSequence();
        Serial.println();
        break;
      
      default:
        // 알 수 없는 명령 무시 (개행 문자 등)
        if (cmd >= 32 && cmd <= 126) {  // 출력 가능한 ASCII 문자만 표시
          Serial.printf("[TEST MODE] Unknown command: '%c' (0x%02X)\n", cmd, cmd);
          Serial.println("[TEST MODE] Available commands: m, c, k, s, p, a");
        }
        break;
    }
  }
