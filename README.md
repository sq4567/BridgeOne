...
#### 5. 무한 스크롤 최적화 설정 (Windows) — 권장

**목적**: 무한 스크롤 모드에서 부드럽고 세밀한 스크롤 경험 제공

**배경**:

BridgeOne의 무한 스크롤 모드는 관성 기반의 연속 스크롤을 제공합니다. 그러나 Windows의 기본 마우스 스크롤 설정(한 번에 3줄)에서는 스크롤 한 칸 한 칸이 큰 단위로 이동하여 뚝뚝 끊기는 느낌이 날 수 있습니다. 스크롤 줄 수를 최소로 낮추면 동일한 입력에서 훨씬 부드러운 스크롤 경험을 얻을 수 있습니다.

**설정 방법**:
1. **Windows 설정 열기**
   ```
   Win + I → Bluetooth 및 장치 → 마우스
   ```

2. **스크롤 줄 수 조정**
   - "한 번에 스크롤할 줄 수" 슬라이더를 **1줄(최솟값)** 로 설정

**효과**:
- 무한 스크롤 관성 구간에서 훨씬 세밀하고 부드러운 스크롤
- 일반 마우스 휠 사용 시에도 더 정밀한 스크롤 가능

> 💡 이 설정은 BridgeOne 전용이 아닌 시스템 전역 설정입니다. 기존 마우스 스크롤 속도가 달라질 수 있으니 참고하세요.
